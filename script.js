...
/* =====================================================
   PVP V5.3.8b — POTS PERSISTENTES + STAGE LOCK VISUAL
   Não altera criação, entrada ou validação de Challenge Room.
===================================================== */
(function(){
  const PVP_SAFE_CARD_KEY="hg_pvp_battle_cards_safe_v1";

  function pvpSafeCards(raw){
    try{
      const value=typeof raw==="string"?JSON.parse(raw):raw;
      return Array.isArray(value)?pvpNormalizarBattleCards(value):null;
    }catch(erro){return null}
  }
  function pvpSafeRead(){
    try{
      const own=pvpSafeCards(localStorage.getItem(PVP_SAFE_CARD_KEY));
      if(own)return own;
      const sources=[localStorage.getItem("hg_pvp_battle_cards_canonical_v1"),localStorage.getItem("hg_pvp_battle_cards_v2"),localStorage.getItem(PVP_BATTLE_CARD_PRESET_KEY)];
      for(const raw of sources){const cards=pvpSafeCards(raw);if(cards&&cards.some(Boolean))return cards}
      const team=JSON.parse(localStorage.getItem(PVP_STORAGE_KEY)||"null");
      const cards=pvpSafeCards(team&&team.battleCards);if(cards&&cards.some(Boolean))return cards;
    }catch(erro){}
    return pvpCriarBattleCardsPadrao();
  }
  function pvpSafeWrite(cards){
    const normal=pvpNormalizarBattleCards(cards);
    pvpBattleCards=normal.slice();
    try{localStorage.setItem(PVP_SAFE_CARD_KEY,JSON.stringify(normal))}catch(erro){}
    // Os leitores antigos continuam recebendo exatamente a mesma seleção.
    ["hg_pvp_battle_cards_canonical_v1","hg_pvp_battle_cards_v2",PVP_BATTLE_CARD_PRESET_KEY].forEach(function(key){try{localStorage.setItem(key,JSON.stringify(normal))}catch(erro){}});
    try{
      const team=JSON.parse(localStorage.getItem(PVP_STORAGE_KEY)||"null");
      if(team&&team.format==="holy-guardians-pvp-team"){team.battleCards=normal.slice();localStorage.setItem(PVP_STORAGE_KEY,JSON.stringify(team))}
    }catch(erro){}
    return normal;
  }
  function pvpSafeRenderCards(){
    const box=document.getElementById("pvpBattleCardLoadout");if(!box)return;
    const cards=pvpSafeRead();pvpBattleCards=cards.slice();
    box.innerHTML=cards.map(function(selected,index){
      const def=selected?PVP_BATTLE_CARD_DEFS[selected]:null;
      const options='<option value="">— SELECT CARD —</option>'+Object.keys(PVP_BATTLE_CARD_DEFS).map(function(key){const item=PVP_BATTLE_CARD_DEFS[key];return '<option value="'+key+'" '+(selected===key?'selected':'')+'>'+pvpEscapeHtml(item.name)+'</option>'}).join("");
      return '<article class="pvp-loadout-card '+(def?'configured':'empty')+'"><div class="pvp-loadout-card-key">F'+(index+1)+'</div><div class="pvp-loadout-card-icon">'+(def?'<img src="'+def.icon+'" alt="">':'<span>+</span>')+'</div><div class="pvp-loadout-card-copy"><strong>'+(def?pvpEscapeHtml(def.name):'BATTLE CARD')+'</strong><small>'+(def?pvpEscapeHtml(def.short):'Escolha a carta para este slot')+'</small><select aria-label="Battle Card F'+(index+1)+'" onchange="pvpSelecionarBattleCard('+(index+1)+',this.value)">'+options+'</select></div></article>';
    }).join("");
    const status=document.getElementById("pvpBattleCardLoadoutStatus");
    if(status){const total=cards.filter(Boolean).length;status.textContent=total+" / 3 EQUIPPED";status.classList.toggle("ok",total===3)}
  }

  // Esta versão não lê chaves antigas durante o render; por isso F5 não apaga a escolha.
  pvpSafeWrite(pvpSafeRead());
  pvpRenderBattleCardLoadout=pvpSafeRenderCards;
  pvpSelecionarBattleCard=function(slotNumero,id){
    const cards=pvpSafeRead(),index=Math.max(0,Math.min(2,Number(slotNumero)-1)),key=String(id||"").toLowerCase();
    cards[index]=PVP_BATTLE_CARD_DEFS[key]?key:null;
    pvpSafeWrite(cards);pvpSafeRenderCards();
    if(typeof pvpMatchAtualizarTeamCheck==="function")pvpMatchAtualizarTeamCheck();
  };

  function pvpSafeVisibleStage(){
    const metas=Array.from(document.querySelectorAll("#pvpSlots .pvp-slot .pvp-slot-meta"));
    if(metas.length!==8)return "";
    const stages=metas.map(function(meta){
      const found=String(meta.textContent||"").toUpperCase().match(/\b(ROOKIE|CHAMPION|ULTIMATE|MEGA)\b/);
      return found&&found[1]||"";
    });
    return stages.every(Boolean)&&new Set(stages).size===1?stages[0]:"";
  }
  const pvpSafeRenderPhase=pvpMatchRenderByPhase;
  pvpMatchRenderByPhase=function(){
    const out=pvpSafeRenderPhase.apply(this,arguments),stage=pvpSafeVisibleStage();
    if(stage){
      const lock=document.getElementById("pvpMatchRoomStage");if(lock)lock.textContent=stage;
      const draft=document.getElementById("pvpDraftStage");if(draft)draft.textContent=stage;
    }
    return out;
  };

  window.addEventListener("pagehide",function(){pvpSafeWrite(pvpBattleCards)});
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",function(){setTimeout(pvpSafeRenderCards,260)});
  else setTimeout(pvpSafeRenderCards,260);
  try{document.documentElement.setAttribute("data-hg-pvp-build","5.3.8b")}catch(erro){}
})();
