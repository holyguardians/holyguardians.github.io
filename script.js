...
/* =====================================================
   PVP V5.3.8 — FIX DEFINITIVO: POTS + STAGE LOCK
   A V5.3.7 ainda relia uma chave antiga enquanto renderizava o loadout.
   Esta camada mantém uma chave final e calcula a Stage pelos oito slots
   visíveis antes de criar qualquer sala.
===================================================== */
(function(){
  const PVP_V538_CARD_KEY="hg_pvp_battle_cards_fixed_v538";

  function pvpV538Cards(raw){
    try{
      const value=typeof raw==="string"?JSON.parse(raw):raw;
      return Array.isArray(value)?pvpNormalizarBattleCards(value):null;
    }catch(erro){return null}
  ...
  function pvpV538ReadCards(){
    try{
      const fixed=localStorage.getItem(PVP_V538_CARD_KEY);
      if(fixed!==null)return pvpV538Cards(fixed)||pvpCriarBattleCardsPadrao();

      // Migração única: ignora chaves vazias antigas que eram gravadas por engano.
      const candidates=[
        localStorage.getItem("hg_pvp_battle_cards_canonical_v1"),
        localStorage.getItem("hg_pvp_battle_cards_v2"),
        localStorage.getItem(PVP_BATTLE_CARD_PRESET_KEY)
      ];
      try{
        const team=JSON.parse(localStorage.getItem(PVP_STORAGE_KEY)||"null");
        if(team&&Array.isArray(team.battleCards))candidates.push(team.battleCards);
      ...
      for(const item of candidates){
        const cards=pvpV538Cards(item);
        if(cards&&cards.some(Boolean))return cards;
      }
    }catch(erro){}
    return pvpCriarBattleCardsPadrao();
  }

  function pvpV538WriteCards(cards){
    const normal=pvpNormalizarBattleCards(cards);
    pvpBattleCards=normal.slice();
    try{localStorage.setItem(PVP_V538_CARD_KEY,JSON.stringify(normal))}catch(erro){}
    // Sincroniza as chaves antigas para que nenhum renderizador legado restaure valor velho.
    ["hg_pvp_battle_cards_canonical_v1","hg_pvp_battle_cards_v2",PVP_BATTLE_CARD_PRESET_KEY].forEach(function(key){
      try{localStorage.setItem(key,JSON.stringify(normal))}catch(erro){}
    });
    try{
      const team=JSON.parse(localStorage.getItem(PVP_STORAGE_KEY)||"null");
      if(team&&team.format==="holy-guardians-pvp-team"){
  ...
  // Faz a migração antes de qualquer render tardio dos patches anteriores.
  pvpV538WriteCards(pvpV538ReadCards());

  // Não chama a cadeia antiga: ela renderiza antes de salvar e apaga a seleção.
  pvpSelecionarBattleCard=function(slotNumero,id){
    const index=Math.max(0,Math.min(2,Number(slotNumero)-1));
    const key=String(id||"").toLowerCase();
    const next=pvpNormalizarBattleCards(pvpBattleCards);
    next[index]=PVP_BATTLE_CARD_DEFS[key]?key:null;
    pvpV538WriteCards(next);
    pvpRenderBattleCardLoadout();
    pvpSalvarEstadoLocal();
    if(typeof pvpMatchAtualizarTeamCheck==="function")pvpMatchAtualizarTeamCheck();
  };

  function pvpV538CanonStage(value){
    const text=String(value||"").trim().toUpperCase();
    return text==="ROOKIE"?"Rookie":text==="CHAMPION"?"Champion":text==="ULTIMATE"?"Ultimate":text==="MEGA"?"Mega":"";
  }

  function pvpV538StageFromSlots(){
    const slots=Array.from(document.querySelectorAll("#pvpSlots .pvp-slot"));
    if(slots.length!==8)return "";
    const stages=slots.map(function(slot){
      const hgid=normalizarHgid(slot&&slot.dataset&&slot.dataset.hgid);
      const digi=hgid&&Array.isArray(pvpDatabase)?pvpDatabase.find(function(item){return mesmoHgid(item&&item.hgid,hgid)}):null;
      let stage=pvpV538CanonStage(digi&&digi.stage);
      if(stage)return stage;
      // Fallback proposital: é a Stage exibida no card que o jogador escolheu.
      const visual=String((slot&&slot.querySelector(".pvp-slot-meta")||{}).textContent||"").toUpperCase();
      const found=visual.match(/\b(ROOKIE|CHAMPION|ULTIMATE|MEGA)\b/);
      return pvpV538CanonStage(found&&found[1]);
    });
    if(stages.some(function(stage){return !stage}))return "";
    const unique=Array.from(new Set(stages));
    return unique.length===1?unique[0]:"";
  ...
  function pvpV538Team(){
    const team=pvpLerEstado();
    const cards=pvpV538ReadCards();
    team.battleCards=cards.slice();
    const stage=pvpV538StageFromSlots();
    if(stage){
      team.stage=stage;
      team.level=PVP_STAGE_LEVEL[stage]||team.level;
      pvpStageAtual=stage;
      const select=document.getElementById("pvpMatchCreateStage");if(select)select.value=stage;
    }
    return {team:team,stage:stage||pvpV538CanonStage(team.stage)||pvpStageAtual};
  }

  // Local Alpha é o fluxo da imagem: fixa o valor no estado da sala, não apenas no texto.
  pvpMatchIniciarTesteLocal=function(){
    const nick=pvpMatchNickValido();if(!nick)return;
    const prepared=pvpV538Team(),team=prepared.team,stage=prepared.stage;
    if(!pvpMatchTeamValido(team,stage)){
      if(typeof pvpMatchAbrirEditorRapido==="function")pvpMatchAbrirEditorRapido("auto",stage);
      else alert("Para o teste local, conclua os 8 builds e equipe 3 Battle Cards.");
      return;
  ...
  // Mantém a mesma fonte de Stage também ao criar uma sala online.
  pvpMatchCriarSala=async function(){
    const nick=pvpMatchNickValido();if(!nick)return;
    const prepared=pvpV538Team(),team=prepared.team,stage=prepared.stage;
    if(!pvpMatchTeamValido(team,stage)){
  ...
  window.addEventListener("pagehide",function(){pvpV538WriteCards(pvpBattleCards)});
  try{document.documentElement.setAttribute("data-hg-pvp-build","5.3.8")}catch(erro){}
  try{console.info("[HG PvP] build 5.3.8 · pots persistence + stage lock")}catch(erro){}
})();
