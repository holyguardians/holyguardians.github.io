...
// Relação adicionada para o novo Imperialdramon até o mapa consolidado ser regenerado.
HG_RELATION_BY_DIGIMON.imperialdramonfightermodeinfected = {
  strong: "Iron",
  strongEffect: "Resistance",
  weak: "Physical",
  weakEffect: "Cannot Evade"
};
...
function formatarNomeCardDigidex(nome) {
  const seguro = escaparHtml(String(nome || "-"));
...
function criarCard(d) {
  ...
  return `
    ...
    <div class="card digidex-profile-trigger${cardMutant ? " is-mutant-digidex" : ""}" role="button" tabindex="0"
      ...
      <h2 class="card-name">
        ${formatarNomeCardDigidex(d.digimon)}
      </h2>
...
const PVP_BATTLE_CARD_PRESET_KEY = "hg_pvp_battle_cards_v1";
function pvpCriarBattleCardsPadrao(){return [null,null,null]}
function pvpNormalizarBattleCards(cards){
  const raw=Array.isArray(cards)?cards:[];
  return [0,1,2].map(function(i){
    const id=String(raw[i]||"").toLowerCase();
    return PVP_BATTLE_CARD_DEFS[id]?id:null;
  });
}
function pvpLerBattleCardsPreset(){
  try{
    const raw=JSON.parse(localStorage.getItem(PVP_BATTLE_CARD_PRESET_KEY)||"null");
    if(!Array.isArray(raw))return null;
    const cards=pvpNormalizarBattleCards(raw);
    return cards.some(Boolean)?cards:null;
  }catch(erro){return null}
}
function pvpSalvarBattleCardsPreset(cards){
  try{localStorage.setItem(PVP_BATTLE_CARD_PRESET_KEY,JSON.stringify(pvpNormalizarBattleCards(cards)))}catch(erro){}
}
let pvpBattleCards=pvpLerBattleCardsPreset()||pvpCriarBattleCardsPadrao();
function pvpBattleCardsCompletos(cards){return pvpNormalizarBattleCards(cards==null?pvpBattleCards:cards).every(Boolean)}
function pvpSelecionarBattleCard(slotNumero,id){
  const index=Math.max(0,Math.min(2,Number(slotNumero)-1));
  const key=String(id||"").toLowerCase();
  pvpBattleCards[index]=PVP_BATTLE_CARD_DEFS[key]?key:null;
  pvpSalvarBattleCardsPreset(pvpBattleCards);
  pvpRenderBattleCardLoadout();
  pvpSalvarEstadoLocal();
  if(typeof pvpMatchAtualizarTeamCheck==="function")pvpMatchAtualizarTeamCheck();
}
function pvpRenderBattleCardLoadout(){
  const box=document.getElementById("pvpBattleCardLoadout");if(!box)return;
  const cards=pvpNormalizarBattleCards(pvpBattleCards);
  box.innerHTML=cards.map(function(selected,index){
    const def=selected?PVP_BATTLE_CARD_DEFS[selected]:null;
    const options='<option value="">— SELECT CARD —</option>'+Object.keys(PVP_BATTLE_CARD_DEFS).map(function(key){
      const item=PVP_BATTLE_CARD_DEFS[key];return '<option value="'+key+'" '+(selected===key?'selected':'')+'>'+pvpEscapeHtml(item.name)+'</option>';
    }).join("");
    return '<article class="pvp-loadout-card '+(def?'configured':'empty')+'"><div class="pvp-loadout-card-key">F'+(index+1)+'</div><div class="pvp-loadout-card-icon">'+(def?'<img src="'+def.icon+'" alt="">':'<span>+</span>')+'</div><div class="pvp-loadout-card-copy"><strong>'+(def?pvpEscapeHtml(def.name):'BATTLE CARD')+'</strong><small>'+(def?pvpEscapeHtml(def.short):'Escolha a carta para este slot')+'</small><select aria-label="Battle Card F'+(index+1)+'" onchange="pvpSelecionarBattleCard('+(index+1)+',this.value)">'+options+'</select></div></article>';
  }).join("");
  const status=document.getElementById("pvpBattleCardLoadoutStatus");if(status){const total=cards.filter(Boolean).length;status.textContent=total+" / 3 EQUIPPED";status.classList.toggle("ok",total===3)}
}
...
function pvpMostrarView(nome){
  ...
  const id=
    ...
    nome==="tournament"?"pvpTournamentView":
...
async function pvpCarregarDatabase(){
  ...
  pvpDadosCarregando=Promise.all([detalhesPromise,masterPromise])
    .then(function(fontes){
      ...
      pvpSincronizarStageRealDoTimeV535();
...
function pvpLerEstado(){
  ...
  return{
    ...
    battleCards:pvpNormalizarBattleCards(pvpBattleCards),
    slots:slots
  }
}
...
function pvpAplicarEstado(pacote){
  ...
  const cardsImportados=pvpNormalizarBattleCards(pacote.battleCards);
  const presetCards=pvpLerBattleCardsPreset();
  pvpBattleCards=cardsImportados.some(Boolean)?cardsImportados:(presetCards||pvpCriarBattleCardsPadrao());
  if(pvpBattleCards.some(Boolean))pvpSalvarBattleCardsPreset(pvpBattleCards);
...
function pvpRestaurarEstadoLocal(){
  try{
    ...
    pvpTeamDeck=pvpNormalizarTeamDeck(deckFonte);
    const presetCards=pvpLerBattleCardsPreset();
    const cardsDoTime=pvpNormalizarBattleCards(pacote&&pacote.battleCards);
    pvpBattleCards=presetCards||cardsDoTime;
    if(!presetCards&&pvpBattleCards.some(Boolean))pvpSalvarBattleCardsPreset(pvpBattleCards);
...
function pvpLimparTimeCompleto(){
  if(!confirm("Limpar todo o time PvP? Isso remove os 8 Digimons, todos os builds, o Buff Deck global e as Battle Cards equipadas."))return;

  pvpTeamDeck=pvpCriarTeamDeckPadrao();
  pvpBattleCards=pvpCriarBattleCardsPadrao();
  ...
  localStorage.removeItem(PVP_BATTLE_CARD_PRESET_KEY);
...
const PVP_MATCH_STREAMER_KEY = "hg_pvp_streamer_mode_v1";
...
let pvpBattlePendingCardSlot = null;
...
function pvpMatchStreamerModeAtivo(){
  try{return localStorage.getItem(PVP_MATCH_STREAMER_KEY)==="1"}catch(erro){return false}
}

function pvpMatchAplicarStreamerMode(){
  const active=pvpMatchStreamerModeAtivo();
  ...
  ["pvpStreamerModeLobbyBtn","pvpStreamerModeWaitingBtn"].forEach(function(id){
    ...
    const state=btn.querySelector("b");if(state)state.textContent=active?"ON":"OFF";
  });
...
function pvpMatchToggleStreamerMode(){
  const next=!pvpMatchStreamerModeAtivo();
...
function pvpStageRealDoTimeV535(){
  const slots=typeof pvpBuildSlots==="function"?pvpBuildSlots():Array.from(document.querySelectorAll("#pvpSlots .pvp-slot"));
  const preenchidos=slots.filter(function(slot){return !!normalizarHgid(slot&&slot.dataset&&slot.dataset.hgid)});
  if(preenchidos.length!==8||!Array.isArray(pvpDatabase)||!pvpDatabase.length)return "";
  const stages=preenchidos.map(function(slot){
    const hgid=normalizarHgid(slot.dataset.hgid);
    const digi=pvpDatabase.find(function(item){return mesmoHgid(item&&item.hgid,hgid)});
    return digi&&PVP_STAGE_LEVEL[digi.stage]?digi.stage:"";
  }).filter(Boolean);
  if(stages.length!==8)return "";
  const unica=Array.from(new Set(stages));
  return unica.length===1?unica[0]:"";
}
function pvpSincronizarStageRealDoTimeV535(){
  const real=pvpStageRealDoTimeV535();
  if(!real)return "";
  pvpStageAtual=real;
  const label=document.getElementById("pvpStageLabel");
...
function pvpMatchTeamValido(team,stage){
  ...
  if(valid.length!==8||!pvpBattleCardsCompletos(team.battleCards))return false;
...
function pvpMatchAtualizarTeamCheck(){
  ...
  const cards=team?pvpNormalizarBattleCards(team.battleCards):pvpCriarBattleCardsPadrao(),cardsTotal=cards.filter(Boolean).length,cardsOk=cardsTotal===3;
  const ready=completos===8&&stageOk&&cardsOk;
  box.innerHTML=
    '<div><span>TEAM</span><b class="'+(completos===8?'ok':'bad')+'">'+completos+'/8 BUILDS</b></div>'+ 
    '<div><span>BATTLE CARDS</span><b class="'+(cardsOk?'ok':'bad')+'">'+cardsTotal+'/3 EQUIPPED</b></div>'+
    ...
    '<div><span>MATCH READY</span><b class="'+(ready?'ok':'bad')+'">'+(ready?'READY':'AJUSTE O TIME')+'</b></div>';
}
...
abrirPvpMatch=function(){
  ...
  pvpCarregarDatabase().then(function(){
    ...
    const realStage=pvpSincronizarStageRealDoTimeV535();
    const savedTeam=pvpMatchTeamAtual();
    const savedStage=realStage||(savedTeam&&savedTeam.stage?savedTeam.stage:pvpStageAtual);
    if(stage)stage.value=savedStage;
    if(realStage)pvpSalvarEstadoLocal();
    pvpMatchAtualizarTeamCheck();
...
async function pvpMatchCriarSala(){
  ...
  const stage=pvpSincronizarStageRealDoTimeV535()||document.getElementById("pvpMatchCreateStage")?.value||pvpStageAtual;
  if(document.getElementById("pvpMatchCreateStage"))document.getElementById("pvpMatchCreateStage").value=stage;
  pvpSalvarEstadoLocal();
  const team=pvpMatchTeamAtual();
  if(!pvpMatchTeamValido(team,stage)){
    alert("Seu time precisa ter 8 builds completos da mesma Stage e 3 Battle Cards equipadas para criar a sala. Use EDITAR TIME para corrigir.");
    return;
  }
  try{
...
async function pvpMatchEntrarSala(){
  ...
  const team=pvpMatchTeamAtual();
  if(!team||!pvpTodosBuildsConcluidos()||!pvpBattleCardsCompletos(team.battleCards)){alert("Conclua os 8 builds e equipe 3 Battle Cards antes de entrar em uma Match.");return}
  try{
...
function pvpMatchRenderWaiting(){
  ...
  const streamer=pvpMatchStreamerModeAtivo();
  const room=document.getElementById("pvpMatchRoomCode");if(room)room.textContent=streamer&&state.roomId?"••••••":(state.roomId||"------");
  ...
  if(invite){
    let realInvite="";
    try{const u=new URL(window.location.href);u.searchParams.set("room",state.roomId||"");realInvite=u.toString()}catch(erro){realInvite=state.roomId||""}
    invite.dataset.realInvite=realInvite;
    invite.value=streamer?"STREAMER MODE // LINK OCULTO":realInvite;
  }
  pvpMatchAplicarStreamerMode();
...
function pvpMatchCopiarConvite(){
  ...
  const real=input.dataset.realInvite||input.value;
  if(navigator.clipboard&&window.isSecureContext)navigator.clipboard.writeText(real).then(function(){alert("Link da Challenge Room copiado!")});
...
function pvpMatchIniciarTesteLocal(){
  ...
  pvpSalvarEstadoLocal();
  const team=pvpMatchTeamAtual();
  if(!pvpMatchTeamValido(team,stage)){alert("Para o teste local, conclua os 8 builds e equipe 3 Battle Cards.");return}
...
function pvpMatchLocalAction(type,payload){
  ...
  }else if(type==="battle_card"&&s.phase==="battle"){
    if(s.battle)pvpBattleApplyCardState(s.battle,role,Number(payload.slot)||0,String(payload.targetId||""));
...
function pvpBattleDeployedUnitsByControl(role){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle;if(!b||!b.units||!b.units[role])return [];
  return b.units[role].filter(function(u){return u.active}).sort(function(a,c){
    const ai=Number.isFinite(Number(a.controlIndex))?Number(a.controlIndex):99,ci=Number.isFinite(Number(c.controlIndex))?Number(c.controlIndex):99;
    return ai-ci||String(a.id).localeCompare(String(c.id));
  });
}
...
function pvpBattleActionHintText(){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,actor=pvpBattleCurrentUnit();if(!b)return "";
  if(pvpBattlePendingCardSlot){const nums=pvpBattleUnitsByControl(pvpBattleMyRole()).map(function(_,i){return String(i+1)});return nums.length?"BATTLE CARD // SELECT ALLY "+nums.join(" · "):"SEM ALVO VÁLIDO"}
  if(!actor)return "";
...
function pvpBattleCardsForRole(role){
  const player=pvpMatchRoomState&&pvpMatchRoomState.players&&pvpMatchRoomState.players[role];
  return pvpNormalizarBattleCards(player&&player.team&&player.team.battleCards);
}
function pvpBattleCardDefinition(role,slotNo){
  const cards=pvpBattleCardsForRole(role),id=cards[Math.max(0,Math.min(2,Number(slotNo)-1))];
  return id?PVP_BATTLE_CARD_DEFS[id]||null:null;
}
function pvpBattleCardTargetNumberForUnit(u){
  if(!pvpBattlePendingCardSlot||!u||u.role!==pvpBattleMyRole()||!u.active||!u.alive)return 0;
  const i=pvpBattleUnitsByControl(u.role).findIndex(function(x){return x.id===u.id});return i>=0?i+1:0;
}
function pvpBattleSelectCard(slotNo){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,me=pvpBattleMyRole();if(!b||b.winner)return;
  const def=pvpBattleCardDefinition(me,slotNo);if(!def)return;
  if(Number(b.gauge&&b.gauge[me]||0)<1){alert("Gauge insuficiente. Battle Cards custam 1 Gauge.");return}
  if(!pvpBattleUnitsByControl(me).length){alert("Nenhum Digimon ativo disponível para receber a Battle Card.");return}
  pvpBattlePendingAction=null;pvpBattlePendingCardSlot=Number(slotNo);
  pvpBattleRenderFields();pvpBattleRenderControls();pvpBattleRenderBattleCards();
}
function pvpBattleUseCardTarget(id){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,me=pvpBattleMyRole(),slotNo=Number(pvpBattlePendingCardSlot)||0;if(!b||!slotNo||b.winner)return;
  const target=pvpBattleUnitById(b,id);if(!target||target.role!==me||!target.active||!target.alive)return;
  if(Number(b.gauge&&b.gauge[me]||0)<1){pvpBattlePendingCardSlot=null;pvpBattleRender();return}
  pvpBattlePendingCardSlot=null;pvpMatchSend("battle_card",{slot:slotNo,targetId:target.id});
  if(!pvpMatchLocalMode){pvpBattleRenderFields();pvpBattleRenderControls();pvpBattleRenderBattleCards()}
}
function pvpBattleApplyCardState(b,role,slotNo,targetId){
  pvpBattleNormalizeBattle(b);const def=pvpBattleCardDefinition(role,slotNo),target=pvpBattleUnitById(b,targetId);
  if(!def||!target||target.role!==role||!target.active||!target.alive||Number(b.gauge[role]||0)<1)return false;
  b.gauge[role]=Math.max(0,Number(b.gauge[role])-1);
...
function pvpBattleRenderBattleCards(){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,box=document.getElementById("pvpBattleCards");if(!box||!b)return;
  const me=pvpBattleMyRole(),cards=pvpBattleCardsForRole(me),gauge=Number(b.gauge&&b.gauge[me]||0);
  box.innerHTML=cards.map(function(id,index){const def=id?PVP_BATTLE_CARD_DEFS[id]:null,selected=Number(pvpBattlePendingCardSlot)===index+1,disabled=!def||gauge<1||!!b.winner;return '<button type="button" class="pvp-battle-card-btn '+(selected?'selected ':'')+'" '+(disabled?'disabled':'')+' onclick="pvpBattleSelectCard('+(index+1)+')">'+(def?'<img src="'+def.icon+'" alt="">':'<span>?</span>')+'<b>F'+(index+1)+'</b><small>'+(def?pvpEscapeHtml(def.short):'EMPTY')+'</small></button>'}).join("");
}
function pvpBattleSelectTarget(id){
  const battle=pvpMatchRoomState&&pvpMatchRoomState.battle;if(!battle)return;
  const u=pvpBattleUnitById(battle,id),me=pvpBattleMyRole();if(!u||!u.active)return;
  if(pvpBattlePendingCardSlot&&u.role===me&&u.alive){pvpBattleUseCardTarget(u.id);return}
  if(!u.alive||u.role===me)return;
  if(pvpBattlePendingAction){
...
function pvpBattleBuffsHtml(u){
  return Object.keys(u&&u.tempBuffs||{}).map(function(key){const bf=u.tempBuffs[key],turns=pvpBattleStatusTurns(bf);if(!bf||turns<=0)return "";const label=bf.type||key,tip=pvpEscapeHtml(label+" · "+turns+" turno"+(turns===1?"":"s")+" restante"+(turns===1?"":"s")+". "+(bf.description||""));return '<span class="pvp-status-icon pvp-card-buff buff-'+String(key).toLowerCase()+'" tabindex="0">'+(bf.icon?'<img src="'+bf.icon+'" alt="">':'<span class="pvp-status-fallback">↑</span>')+'<b>'+turns+'</b><span class="pvp-status-tooltip">'+tip+'</span></span>'}).join("");
}
...
function pvpBattleUnitHtml(u,current,targeted){
  const hp=Math.max(0,Math.min(100,u.hp/u.maxHp*100)),sp=Math.max(0,Math.min(100,u.sp/u.maxSp*100)),skillNum=pvpBattleTargetNumberForUnit(u),cardNum=pvpBattleCardTargetNumberForUnit(u),num=cardNum||skillNum;
  let validTarget=!!cardNum;
  if(skillNum&&pvpBattlePendingAction){
    const actor=pvpBattleCurrentUnit(),d=actor&&pvpMatchDigi(actor.hgid),skill=d&&(d.skills||[]).find(function(s){return Number(s.slot)===Number(pvpBattlePendingAction.slot)});
    validTarget=!!(actor&&skill&&pvpBattleTargetValid(actor,u,skill));
  }
  return '<div class="pvp-battle-unit '+(u.position==="F"?'front':'back')+' '+(!u.alive?'is-dead ':'')+pvpBattleUnitStatusClasses(u)+(current?' current':'')+(targeted?' targeted':'')+'" onclick="pvpBattleSelectTarget(\''+u.id+'\')">'+
    (num&&validTarget?'<button type="button" class="pvp-target-number valid '+(cardNum?'card-target':'')+'" onclick="event.stopPropagation();'+(cardNum?'pvpBattleUseCardTarget(\''+u.id+'\')':'pvpBattleSelectTargetNumber('+num+')')+'">'+num+'</button>':'')+
    '<div class="pvp-battle-unit-frame"><span class="pvp-battle-unit-pos">'+u.position+'</span><img src="'+u.icon+'" alt="">'+(!u.alive?'<span class="pvp-dead-label">DEFEATED</span>':'')+'</div><strong class="pvp-battle-unit-name">'+pvpEscapeHtml(u.name)+'</strong>'+
    '<div class="pvp-battle-unit-hp"><i style="width:'+hp+'%"></i></div><div class="pvp-battle-unit-sp"><i style="width:'+sp+'%"></i></div><div class="pvp-battle-unit-status">'+pvpBattleStatusesHtml(u)+pvpBattleBuffsHtml(u)+'</div></div>';
}
...
function pvpBattleRenderTarget(){
  ...
  box.innerHTML='<div class="pvp-target-top"><div class="pvp-target-copy"><div class="pvp-target-name-line">'+pvpTypeIconHtml(d.attribute||"UNKNOWN")+'<strong>'+pvpEscapeHtml(target.name)+'</strong></div><small>'+target.position+' · '+pvpEscapeHtml(pvpMatchRoomState.players[target.role].nick)+'</small></div></div>'+
    ...
    '<div class="pvp-target-hpbar"><i style="width:'+pct+'%"></i></div><div class="pvp-target-hp-number">'+Math.max(0,Math.round(target.hp)).toLocaleString("pt-BR")+' / '+Math.round(target.maxHp).toLocaleString("pt-BR")+'</div><div class="pvp-target-debuffs">'+pvpBattleStatusesHtml(target)+pvpBattleBuffsHtml(target)+'</div>';
}
...
function pvpBattleSelectSkill(hgid,slotNo,isBurst){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,me=pvpBattleMyRole(),actor=pvpBattleCurrentUnit();if(!b||!actor||actor.role!==me||!mesmoHgid(actor.hgid,hgid)||pvpBattleCcBlocksCommands(actor))return;
...
function pvpBattleRenderControls(){
  ...
  const standby=(b.units[me]||[]).some(function(u){return !u.active&&u.alive}),deployed=(b.units[me]||[]).some(function(u){return u.active}),sub=document.getElementById("pvpSubstitutionBtn");if(sub)sub.disabled=b.subs[me]<=0||gauge<1||!standby||!deployed||!!b.winner;
...
function pvpBattleRender(){
  ...
  pvpBattleRenderOpponentLabel();pvpBattleRenderTarget();pvpBattleRenderFields();pvpBattleRenderTurnQueue();pvpBattleRenderSkills();pvpBattleRenderLog();pvpBattleRenderControls();pvpBattleRenderBattleCards();
...
function pvpBattleTickActorEnd(actor){actor.status.cc=pvpBattleTickStatus(actor.status.cc);actor.status.dot=pvpBattleTickStatus(actor.status.dot);actor.status.defBreak=pvpBattleTickStatus(actor.status.defBreak);Object.keys(actor.tempBuffs||{}).forEach(function(k){const bf=actor.tempBuffs[k];if(bf&&bf.turns>0)bf.turns--;if(!bf||bf.turns<=0)delete actor.tempBuffs[k]})}
function pvpBattleHandleDeath(unit){
  const b=pvpMatchRoomState.battle;unit.hp=0;unit.alive=false;unit.active=true;pvpBattleIntentByUnit[unit.id]=null;pvpBattleLog(unit.name+" was defeated. The field slot remains occupied until a manual substitution.");
  pvpBattlePendingAction=null;pvpBattlePendingCardSlot=null;pvpBattleRebuildOrder(b,false);pvpBattleCheckWinner();
}
function pvpBattleCheckWinner(){const b=pvpMatchRoomState.battle;["host","guest"].forEach(function(role){if(!b.units[role].some(function(u){return u.active&&u.alive}))b.winner=pvpMatchOpponentRole(role)})}
function pvpBattleAdvanceTurn(actor,chargeBurst){
  ...
  b.turnSerial=(Number(b.turnSerial)||0)+1;b.preparedTurnSerial=-1;pvpBattlePendingAction=null;pvpBattlePendingCardSlot=null;pvpBattleSelectedTarget=null;
}
...
function pvpBattlePrepareCurrentTurn(){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,actor=pvpBattleCurrentUnit();if(!b||!actor||b.winner)return;
...
function pvpBattleResolveSelectedAction(targetId){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,actor=pvpBattleCurrentUnit(),me=pvpBattleMyRole();if(!b||!actor||b.winner||actor.role!==me||!pvpBattlePendingAction||pvpBattlePendingAction.unitId!==actor.id)return;
...
function pvpBattleAbrirSubstituicao(){
  ...
  if(!(b.units[me]||[]).some(function(u){return u.active})){alert("Nenhum slot de campo disponível para substituir.");return}
  pvpBattlePendingCardSlot=null;pvpBattleSubMode="manual";pvpBattleSubOut=null;pvpBattleSubIn=null;const overlay=document.getElementById("pvpSubstitutionOverlay");overlay.classList.add("aberto");overlay.setAttribute("aria-hidden","false");pvpBattleRenderSubstitution();
}
function pvpBattleFecharSubstituicao(){
  const o=document.getElementById("pvpSubstitutionOverlay");if(o){o.classList.remove("aberto");o.setAttribute("aria-hidden","true")}
...
function pvpBattleRenderSubstitution(){
  ...
  function card(u,kind){
    const selected=(kind==="out"?pvpBattleSubOut:pvpBattleSubIn)===u.id,dead=!u.alive,disabled=kind==="in"&&dead,hpPct=Math.max(0,Math.min(100,(u.hp/u.maxHp)*100)),spPct=Math.max(0,Math.min(100,(u.sp/u.maxSp)*100));
    return '<button type="button" class="pvp-sub-card '+(selected?'selecionado ':'')+(dead?'morto ':'')+'" '+(disabled?'disabled':'')+' onclick="pvpBattleSelectSub(\''+kind+'\',\''+u.id+'\')"><img src="'+u.icon+'" alt=""><strong>'+pvpEscapeHtml(u.name)+'</strong><small>'+(dead?'DEFEATED · ':'')+u.position+' · BURST '+u.burst+'/5</small><span class="pvp-sub-hp"><i style="width:'+hpPct+'%"></i><b>'+Math.max(0,Math.round(u.hp)).toLocaleString("pt-BR")+' / '+Math.round(u.maxHp).toLocaleString("pt-BR")+'</b></span><span class="pvp-sub-sp"><i style="width:'+spPct+'%"></i><b>'+Math.max(0,Math.round(u.sp)).toLocaleString("pt-BR")+' / '+Math.round(u.maxSp).toLocaleString("pt-BR")+'</b></span></button>';
  }
  const deployUnits=(b.units[me]||[]).filter(function(u){return u.active}).sort(function(a,c){return Number(a.controlIndex)-Number(c.controlIndex)});
  deploy.innerHTML=deployUnits.map(function(u){return card(u,"out")}).join("");
  standby.innerHTML=(b.units[me]||[]).filter(function(u){return !u.active&&u.alive}).map(function(u){return card(u,"in")}).join("")||'<div class="pvp-sub-empty">Nenhum Standby vivo disponível.</div>';
  const ks=document.getElementById("pvpSubModeKicker"),tt=document.getElementById("pvpSubModeTitle"),close=document.getElementById("pvpSubCloseBtn");if(ks)ks.textContent="SUBSTITUTION";if(tt)tt.textContent="TROCAR DIGIMON";if(close)close.style.display="";
  const c=document.getElementById("pvpSubModalCount");if(c)c.textContent=b.subs[me]+"/3";const foot=document.getElementById("pvpSubFootText");if(foot)foot.innerHTML='Remaining Substitution Count <b>'+b.subs[me]+'/3</b> · Cost 1 Gauge';
  const btn=document.getElementById("pvpSubConfirmBtn");if(btn){btn.textContent="SUBSTITUIR";btn.disabled=!pvpBattleSubOut||!pvpBattleSubIn}
}
function pvpBattleSelectSub(kind,id){if(kind==="out")pvpBattleSubOut=id;else pvpBattleSubIn=id;pvpBattleRenderSubstitution()}
function pvpBattleApplySubstitutionState(b,role,outId,inId){
  pvpBattleNormalizeBattle(b);const out=pvpBattleUnitById(b,outId),incoming=pvpBattleUnitById(b,inId);if(!out||!incoming||out.role!==role||incoming.role!==role||!out.active||incoming.active||!incoming.alive||b.subs[role]<=0||Number(b.gauge[role]||0)<1)return false;
  const currentId=b.turnOrder&&b.turnOrder[b.turnIndex],wasCurrent=currentId===out.id,pos=out.position,control=Number(out.controlIndex),wasDead=!out.alive;out.active=false;out.controlIndex=null;incoming.active=true;incoming.position=pos;incoming.controlIndex=Number.isFinite(control)?control:0;b.subs[role]--;b.gauge[role]=Math.max(0,Number(b.gauge[role])-1);
  pvpBattleLog((wasDead?out.name+"'s defeated slot was replaced by ":out.name+" was substituted by ")+incoming.name+". Cost 1 Gauge · Remaining substitutions "+b.subs[role]+"/3.");pvpBattleRebuildOrder(b,false);if(wasCurrent&&b.turnOrder.includes(incoming.id)){b.turnIndex=b.turnOrder.indexOf(incoming.id);b.preparedTurnSerial=-1;pvpBattlePendingAction=null}else if(currentId&&b.turnOrder.includes(currentId))b.turnIndex=b.turnOrder.indexOf(currentId);pvpBattleCheckWinner();return true;
}
function pvpBattleConfirmarSubstituicao(){
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,me=pvpBattleMyRole();if(!b||!pvpBattleSubOut||!pvpBattleSubIn||b.subs[me]<=0||Number(b.gauge[me]||0)<1)return;const outId=pvpBattleSubOut,inId=pvpBattleSubIn;pvpBattleFecharSubstituicao();pvpMatchSend("substitute",{outId:outId,inId:inId});
}
...
function pvpMatchKeyboard(event){
  ...
  const b=pvpMatchRoomState&&pvpMatchRoomState.battle,me=pvpBattleMyRole(),actor=pvpBattleCurrentUnit();if(!b||b.winner)return;
  const key=String(event.key||"").toUpperCase();
  if(/^F[1-3]$/.test(key)){event.preventDefault();pvpBattleSelectCard(Number(key.slice(1)));return}
  if(pvpBattlePendingCardSlot&&["1","2","3"].includes(key)){event.preventDefault();const target=pvpBattleUnitsByControl(me)[Number(key)-1];if(target)pvpBattleUseCardTarget(target.id);return}
  if(pvpBattlePendingAction&&["1","2","3"].includes(key)){event.preventDefault();pvpBattleSelectTargetNumber(Number(key));return}
  if(!actor||actor.role!==me)return;const index=Number.isFinite(Number(actor.controlIndex))?Number(actor.controlIndex):0,keys=pvpBattleSkillKeys(index),idx=keys.indexOf(key);if(idx<0)return;
...
const HG_TIERLIST_CUSTOM_KEY = "hgTierListDsrCustomV1";
...
function tierListLerCustom() {
  try {
    const valor = JSON.parse(localStorage.getItem(HG_TIERLIST_CUSTOM_KEY) || "[]");
...
function tierListSalvarCustom(lista) {
  try {
    localStorage.setItem(HG_TIERLIST_CUSTOM_KEY, JSON.stringify(lista || []));
    return true;
  } catch (erro) {
    console.error("Tier List DSR custom storage:", erro);
...
function tierListMontarCatalogo() {
  ...
  tierListLerCustom().forEach(function(item) {
    ...
    mapa.set(key, {
      ...
      icon: icon,
      fallback: "",
      stage: normalizarStageDigidex(item && item.stage) || "MEGA",
      type: normalizarType(item && item.type) || "",
      field: "",
      hp: "",
      sp: "",
      str: "",
...
function tierListCardHtml(key) {
  ...
  return `
    <article class="tierlist-digi" data-key="${tierListEscAttr(digi.key)}" data-name="${tierListEscAttr(tierListNormalizarTexto(digi.name))}" data-stage="${tierListEscAttr(digi.stage)}" data-type="${tierListEscAttr(digi.type)}" data-custom="${digi.custom ? "1" : "0"}" onmouseenter="tierListMostrarTooltip(this)" onmouseleave="tierListOcultarTooltip()">
      ...
      ${digi.custom ? `<button type="button" class="tierlist-dsr-custom-delete" data-html2canvas-ignore="true" title="Excluir ícone personalizado" aria-label="Excluir ${tierListEscAttr(digi.name)}" onclick="tierListExcluirCustom('${tierListEscAttr(digi.key)}', event)">⌫</button>` : ""}
...
function tierListResetar() {
  if (!confirm("Resetar a Tier List DSR? As tiers personalizadas e posições salvas serão apagadas. Seus ícones enviados permanecerão disponíveis.")) return;
...
function tierListAbrirUpload() {
  const input = document.getElementById("tierListUploadInput");
  if (!input) return;
  input.value = "";
  input.click();
}

function tierListReceberArquivo(file) {
  if (!file) return;
  ...
  reader.onload = function() {
    ...
    img.onload = function() {
      ...
      tierListUploadData = canvas.toDataURL("image/webp", .88);

      const modal = document.getElementById("tierListUploadModal");
      const preview = document.getElementById("tierListUploadPreview");
      const vazio = document.getElementById("tierListUploadPreviewEmpty");
      const nome = document.getElementById("tierListUploadName");
      if (preview) { preview.src = tierListUploadData; preview.hidden = false; }
...
function tierListConfirmarUpload() {
  const nome = String(document.getElementById("tierListUploadName")?.value || "").trim();
  const stage = normalizarStageDigidex(document.getElementById("tierListUploadStage")?.value || "MEGA") || "MEGA";
  const type = normalizarType(document.getElementById("tierListUploadType")?.value || "") || "";
  if (!tierListUploadData) {
    alert("Escolha uma imagem primeiro.");
    return;
  }
  if (!nome) {
    alert("Digite o nome do Digimon.");
    document.getElementById("tierListUploadName")?.focus();
    return;
  }

  const lista = tierListLerCustom();
  const key = "custom:dsr:" + Date.now().toString(36) + ":" + Math.random().toString(36).slice(2, 7);
  lista.push({ key: key, name: nome.slice(0, 60), stage: stage, type: type, icon: tierListUploadData });
  if (!tierListSalvarCustom(lista)) return;

  tierListFecharUpload();
  tierListSincronizarDatabase();
}

function tierListExcluirCustom(key, event) {
  if (event) { event.preventDefault(); event.stopPropagation(); }
  const digi = tierListCatalogoMap.get(String(key));
  if (!digi || !digi.custom) return;
  if (!confirm(`Excluir o ícone personalizado "${digi.name}" deste navegador?`)) return;

  const lista = tierListLerCustom().filter(function(item) { return String(item.key) !== String(key); });
  if (!tierListSalvarCustom(lista)) return;

  tierListEstado.tiers.forEach(function(tier) {
    tier.items = tier.items.filter(function(item) { return item !== key; });
  });
  tierListEstado.pool = tierListEstado.pool.filter(function(item) { return item !== key; });
  tierListSalvarEstado(true);
  tierListSincronizarDatabase();
}
...
function inicializarTierListDsr() {
  ...
  if (!tierListInicializada) {
    ...
    document.addEventListener("keydown", function(event) {
      if (event.key === "Escape" && !document.getElementById("tierListUploadModal")?.hidden) tierListFecharUpload();
    });
  }
...
function hgTournamentLanguage(){
  try{
    ...
    if(urlLang==="en"||urlLang==="en-US")return "en-US";
    if(urlLang==="ko"||urlLang==="kr"||urlLang==="ko-KR")return "ko-KR";
    if(urlLang==="pt"||urlLang==="pt-BR")return "pt-BR";
  }catch(erro){}
  try{
    const saved=localStorage.getItem("hg_language");
    if(saved==="en-US"||saved==="ko-KR"||saved==="pt-BR")return saved;
  }catch(erro){}
  return "pt-BR";
}

function hgTournamentT(key,fallback,values){
  let text=fallback==null?String(key||""):String(fallback);
  try{
    const lang=hgTournamentLanguage();
    const active=window.HG_I18N&&window.HG_I18N[lang];
    const base=window.HG_I18N&&window.HG_I18N["pt-BR"];
    if(active&&Object.prototype.hasOwnProperty.call(active,key))text=active[key];
    else if(base&&Object.prototype.hasOwnProperty.call(base,key))text=base[key];
  }catch(erro){}
  Object.keys(values||{}).forEach(function(name){
    text=String(text).split("{"+name+"}").join(String(values[name]==null?"":values[name]));
  });
  return String(text);
}

function hgTournamentEscape(value){
...
function hgTournamentNormalizeId(value){
  return String(value||"").toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,8);
}

function hgTournamentSanitizeId(input){
  if(input)input.value=hgTournamentNormalizeId(input.value);
}

function hgTournamentSanitizeNick(input){
...
function hgTournamentGetSession(id){
  const key=hgTournamentNormalizeId(id);
  const sessions=hgTournamentReadSessions();
  const value=sessions[key];
  return value&&typeof value==="object"?value:{};
}

function hgTournamentSaveSession(id,patch){
  const key=hgTournamentNormalizeId(id);
  if(!/^HG[A-Z2-9]{6}$/.test(key))return {};
  const sessions=hgTournamentReadSessions();
  const current=sessions[key]&&typeof sessions[key]==="object"?sessions[key]:{};
  sessions[key]=Object.assign({},current,patch||{},{updatedAt:new Date().toISOString()});
  hgTournamentWriteSessions(sessions);
  return sessions[key];
}

function hgTournamentSetActive(id){
  const key=hgTournamentNormalizeId(id);
  try{
...
function hgTournamentReadMatchContext(){
  try{
    const data=JSON.parse(localStorage.getItem(HG_TOURNAMENT_MATCH_KEY)||"null");
    return data&&typeof data==="object"?data:null;
  }catch(erro){return null}
}

function hgTournamentSaveMatchContext(data){
  hgTournamentMatchContext=data&&typeof data==="object"?data:null;
  try{
    if(hgTournamentMatchContext)localStorage.setItem(HG_TOURNAMENT_MATCH_KEY,JSON.stringify(hgTournamentMatchContext));
    else localStorage.removeItem(HG_TOURNAMENT_MATCH_KEY);
  }catch(erro){}
}

function hgTournamentQueryId(){
  try{return hgTournamentNormalizeId(new URL(window.location.href).searchParams.get("tournament")||"")}catch(erro){return ""}
}

function hgTournamentSetQuery(id){
  try{
    const url=new URL(window.location.href);
    const key=hgTournamentNormalizeId(id);
    if(key)url.searchParams.set("tournament",key);
    else url.searchParams.delete("tournament");
    history.replaceState(history.state||{},"",url.toString());
  }catch(erro){}
}

function hgTournamentInviteUrl(id){
  try{
    const url=new URL(window.location.href);
    url.searchParams.set("tournament",hgTournamentNormalizeId(id));
...
function hgTournamentNotice(message,type){
  const box=document.getElementById("hgTournamentNotice");
  if(!box)return;
  const text=String(message||"").trim();
  box.hidden=!text;
  box.className="pvp-tournament-notice"+(type?" "+type:"");
  box.textContent=text;
}

async function hgTournamentRequest(path,options){
  ...
  const response=await fetch(HG_TOURNAMENT_API_URL+path,Object.assign({},config,{cache:"no-store"}));
  let data={};
  try{data=await response.json()}catch(erro){}
  if(!response.ok||data.ok===false){
    const error=new Error(data.message||data.error||hgTournamentT("tournament.error.generic","Não foi possível concluir a operação."));
...
function hgTournamentSetBusy(busy){
  hgTournamentLoading=!!busy;
  ["hgTournamentCreateBtn","hgTournamentJoinBtn"].forEach(function(id){
    const btn=document.getElementById(id);
    if(btn)btn.disabled=hgTournamentLoading;
  });
}

function hgTournamentStatusLabel(status){
  const key="tournament.status."+String(status||"").toLowerCase();
  const fallback={
    registration:"INSCRIÇÕES ABERTAS",
    bracket_ready:"CHAVE PRONTA",
    in_progress:"EM ANDAMENTO",
    finished:"FINALIZADO"
  }[String(status||"").toLowerCase()]||String(status||"-").toUpperCase();
  return hgTournamentT(key,fallback);
}

function hgTournamentPlayerStatusLabel(status){
  const key="tournament.playerStatus."+String(status||"").toLowerCase();
  const fallback={
    registered:"INSCRITO",
    ready:"TIME CONFIRMADO",
...
function hgTournamentStageLabel(stage){
  const level={Rookie:15,Champion:60,Ultimate:90,Mega:100}[stage]||"";
  return String(stage||"-").toUpperCase()+(level?" · LV. "+level:"");
}

function hgTournamentStatusClass(status){
  return String(status||"").toLowerCase().replace(/[^a-z0-9_-]/g,"");
}

function hgTournamentFindPlayer(tournament,session){
  if(!tournament||!session||!session.nick)return null;
  const nick=String(session.nick).toLowerCase();
  return (tournament.players||[]).find(function(player){
    return String(player&&player.nick||"").toLowerCase()===nick;
  })||null;
}

function hgTournamentFlattenMatches(tournament){
  const rounds=tournament&&tournament.bracket&&Array.isArray(tournament.bracket.rounds)
    ?tournament.bracket.rounds:[];
  return rounds.flatMap(function(round){
    return (round.matches||[]).map(function(match){
      return Object.assign({roundNumber:round.round},match);
    });
  });
}

function hgTournamentCurrentMatch(tournament,player){
  if(!tournament||!player)return null;
  const matches=hgTournamentFlattenMatches(tournament).filter(function(match){
    return Number(match.player1&&match.player1.id)===Number(player.id)||
      Number(match.player2&&match.player2.id)===Number(player.id);
  });
  const ready=matches.find(function(match){return !match.winner&&match.status==="ready"&&match.player1&&match.player2});
  if(ready)return ready;
  const pending=matches.find(function(match){return !match.winner&&match.status==="pending"});
  if(pending)return pending;
  return matches.slice().reverse().find(function(match){return !match.winner})||null;
}

function hgTournamentLocalTeamSummary(requiredStage){
  try{
    pvpCriarSlots();
    const team=pvpMatchTeamAtual();
    const slots=team&&Array.isArray(team.slots)?team.slots:[];
    const builds=slots.filter(function(slot){return slot&&slot.hgid&&slot.build&&slot.build.complete}).length;
    const cards=team?pvpNormalizarBattleCards(team.battleCards).filter(Boolean).length:0;
    const stage=team&&team.stage?team.stage:"";
    return {
      team:team,
      builds:builds,
      cards:cards,
      stage:stage,
      stageOk:stage===requiredStage,
      ready:builds===8&&cards===3&&stage===requiredStage
    };
  }catch(erro){
    return {team:null,builds:0,cards:0,stage:"",stageOk:false,ready:false};
  }
}

function hgTournamentRenderSaved(){
  const box=document.getElementById("hgTournamentSavedSessions");
  if(!box)return;
  const sessions=hgTournamentReadSessions();
  const ids=Object.keys(sessions).filter(function(id){return /^HG[A-Z2-9]{6}$/.test(id)});
  if(!ids.length){box.innerHTML="";return}
  ids.sort(function(a,b){
    return String(sessions[b].updatedAt||"").localeCompare(String(sessions[a].updatedAt||""));
  });
  box.innerHTML=
    '<div class="pvp-tournament-saved-head"><small>'+hgTournamentEscape(hgTournamentT("tournament.savedKicker","LOCAL // SAVED"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.savedTitle","SESSÕES SALVAS"))+'</strong></div>'+
    '<div class="pvp-tournament-saved-grid">'+
    ids.slice(0,8).map(function(id){
      const session=sessions[id]||{};
      const role=session.organizerToken
        ?hgTournamentT("tournament.savedOrganizer","ORGANIZADOR")
        :(session.playerToken?hgTournamentT("tournament.savedPlayer","JOGADOR"):hgTournamentT("tournament.savedViewer","VISUALIZAÇÃO"));
      const who=session.nick||session.organizerNick||"";
      return '<button type="button" class="pvp-tournament-saved-card" onclick="hgTournamentOpen(\''+id+'\')">'+
        '<b>'+hgTournamentEscape(id)+'</b>'+
        '<span>'+hgTournamentEscape(role)+(who?' · '+hgTournamentEscape(who):'')+'</span>'+
      '</button>';
    }).join("")+
    '</div>';
}

function hgTournamentRenderLanding(){
  const landing=document.getElementById("hgTournamentLanding");
  const dashboard=document.getElementById("hgTournamentDashboard");
  if(landing)landing.hidden=false;
  if(dashboard)dashboard.hidden=true;
  const nick=document.getElementById("hgTournamentCreateOrganizer");
  if(nick&&!nick.value){
    try{nick.value=String(localStorage.getItem(PVP_MATCH_NICK_KEY)||"").replace(/[^A-Za-z0-9]/g,"").slice(0,16)}catch(erro){}
  }
  const joinNick=document.getElementById("hgTournamentJoinNick");
  if(joinNick&&!joinNick.value&&nick&&nick.value)joinNick.value=nick.value;
  hgTournamentRenderSaved();
}

function hgTournamentRenderTop(tournament,session){
  const box=document.getElementById("hgTournamentDashboardTop");
  if(!box)return;
  const stage=tournament.rules&&tournament.rules.stage?tournament.rules.stage:"-";
  const invite=hgTournamentInviteUrl(tournament.id);
  box.innerHTML=
    '<div class="pvp-tournament-title-block">'+
      '<div class="pvp-tournament-id-line"><span>'+hgTournamentEscape(tournament.id)+'</span><b class="pvp-tournament-status '+hgTournamentStatusClass(tournament.status)+'">'+hgTournamentEscape(hgTournamentStatusLabel(tournament.status))+'</b></div>'+
      '<h3>'+hgTournamentEscape(tournament.name)+'</h3>'+
      '<p>'+hgTournamentEscape(hgTournamentT("tournament.organizedBy","Organizado por {name}",{name:tournament.organizer}))+' · '+hgTournamentEscape(hgTournamentStageLabel(stage))+'</p>'+
    '</div>'+
    '<div class="pvp-tournament-top-actions">'+
      '<button type="button" class="pvp-action-btn pvp-action-secondary" onclick="hgTournamentCopyInvite()">'+hgTournamentEscape(hgTournamentT("tournament.copyInvite","COPIAR CONVITE"))+'</button>'+
      '<button type="button" class="pvp-action-btn pvp-action-secondary" onclick="hgTournamentCopyCode()">'+hgTournamentEscape(hgTournamentT("tournament.copyCode","COPIAR ID"))+'</button>'+
    '</div>'+
    '<div class="pvp-tournament-stats">'+
      '<div><small>'+hgTournamentEscape(hgTournamentT("tournament.playersStat","JOGADORES"))+'</small><strong>'+Number(tournament.playerCount||0)+' / '+Number(tournament.maxPlayers||0)+'</strong></div>'+
      '<div><small>'+hgTournamentEscape(hgTournamentT("tournament.readyStat","TIMES PRONTOS"))+'</small><strong>'+Number(tournament.readyCount||0)+'</strong></div>'+
      '<div><small>'+hgTournamentEscape(hgTournamentT("tournament.stageStat","REGRA DE STAGE"))+'</small><strong>'+hgTournamentEscape(hgTournamentStageLabel(stage))+'</strong></div>'+
    '</div>';
}

function hgTournamentRenderPlayerPanel(tournament,session){
  const box=document.getElementById("hgTournamentPlayerPanel");
  if(!box)return;
  const player=hgTournamentFindPlayer(tournament,session);
  const stage=tournament.rules&&tournament.rules.stage?tournament.rules.stage:"";
  if(!session.playerToken||!player){
    if(tournament.status==="registration"){
      box.innerHTML=
        '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.playerKicker","PLAYER // REGISTRATION"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.joinThis","ENTRAR NESTE TORNEIO"))+'</strong></div></div>'+
        '<div class="pvp-tournament-inline-join">'+
          '<label class="pvp-tournament-field"><span>'+hgTournamentEscape(hgTournamentT("tournament.nick","SEU NICK"))+'</span><input id="hgTournamentInlineNick" maxlength="16" autocomplete="off" oninput="hgTournamentSanitizeNick(this)" placeholder="PlayerOne"></label>'+
          '<button type="button" class="pvp-action-btn pvp-action-success" onclick="hgTournamentJoinActive()">'+hgTournamentEscape(hgTournamentT("tournament.joinButton","ENTRAR"))+'</button>'+
        '</div>'+
        '<p class="pvp-tournament-muted">'+hgTournamentEscape(hgTournamentT("tournament.spectatorHint","Você está visualizando o torneio sem uma inscrição vinculada neste navegador."))+'</p>';
    }else{
      box.innerHTML=
        '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.playerKicker","PLAYER // ACCESS"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.spectator","MODO ESPECTADOR"))+'</strong></div></div>'+
        '<p class="pvp-tournament-muted">'+hgTournamentEscape(hgTournamentT("tournament.registrationClosedViewer","As inscrições já foram encerradas. O chaveamento continua público para visualização."))+'</p>';
    }
    return;
  }

  const local=hgTournamentLocalTeamSummary(stage);
  const isLocked=!!player.teamLocked;
  const stateClass=isLocked?"ok":(local.ready?"warn":"bad");
  let actionHtml="";
  if(isLocked){
    actionHtml='<div class="pvp-tournament-locked-banner"><span>✓</span><div><strong>'+hgTournamentEscape(hgTournamentT("tournament.teamLocked","TIME TRAVADO"))+'</strong><small>'+hgTournamentEscape(hgTournamentT("tournament.teamLockedHint","Este é o time oficial do jogador até a eliminação ou o título."))+'</small></div></div>';
  }else if(tournament.status==="registration"){
    actionHtml=
      '<div class="pvp-tournament-team-check">'+
        '<div><span>TEAM</span><b class="'+(local.builds===8?"ok":"bad")+'">'+local.builds+'/8 BUILDS</b></div>'+
        '<div><span>BATTLE CARDS</span><b class="'+(local.cards===3?"ok":"bad")+'">'+local.cards+'/3</b></div>'+
        '<div><span>STAGE</span><b class="'+(local.stageOk?"ok":"bad")+'">'+hgTournamentEscape(local.stage?hgTournamentStageLabel(local.stage):"-")+'</b></div>'+
      '</div>'+
      '<div class="pvp-tournament-player-actions">'+
        '<button type="button" class="pvp-action-btn pvp-action-secondary" onclick="hgTournamentEditTeam()">'+hgTournamentEscape(hgTournamentT("tournament.editTeam","MONTAR / EDITAR TIME"))+'</button>'+
        '<button type="button" class="pvp-action-btn pvp-action-success" onclick="hgTournamentConfirmTeam()" '+(local.ready?'':'disabled')+'>'+hgTournamentEscape(hgTournamentT("tournament.confirmTeam","CONFIRMAR E TRAVAR TIME"))+'</button>'+
      '</div>';
  }else{
    actionHtml='<div class="pvp-tournament-alert bad">'+hgTournamentEscape(hgTournamentT("tournament.notReadyExcluded","O time não foi confirmado antes do fechamento das inscrições e ficou fora da chave."))+'</div>';
  }

  box.innerHTML=
    '<div class="pvp-tournament-panel-title">'+
      '<div><small>'+hgTournamentEscape(hgTournamentT("tournament.playerKicker","PLAYER // YOUR ENTRY"))+'</small><strong>'+hgTournamentEscape(player.nick)+'</strong></div>'+
      '<b class="pvp-tournament-player-state '+hgTournamentStatusClass(player.status)+'">'+hgTournamentEscape(hgTournamentPlayerStatusLabel(player.status))+'</b>'+
    '</div>'+
    '<div class="pvp-tournament-player-summary '+stateClass+'">'+
      '<span>'+hgTournamentEscape(hgTournamentT("tournament.requiredStage","STAGE OBRIGATÓRIA"))+'</span><strong>'+hgTournamentEscape(hgTournamentStageLabel(stage))+'</strong>'+
    '</div>'+
    actionHtml;
}
...
function hgTournamentRenderOrganizerPanel(tournament,session){
  ...
  if(tournament.status==="registration"){
    ...
    if(!organizerIsPlayer){
      playerControl='<div class="pvp-tournament-organizer-player">'+
        '<p>'+hgTournamentEscape(hasPlayerSlot
          ?hgTournamentT("tournament.organizerNotPlayer","Você criou este torneio como organizador, mas ainda não está inscrito como jogador.")
          :hgTournamentT("tournament.organizerFull","Você criou este torneio como organizador, mas ele já está lotado e não há vaga de jogador para este navegador."))+'</p>'+
...
function hgTournamentRenderParticipants(tournament,session){
  ...
  const myNick=String(session.nick||"").toLowerCase();
  box.innerHTML=players.map(function(player,index){
    const mine=String(player.nick||"").toLowerCase()===myNick;
    return '<div class="pvp-tournament-player-row '+(mine?"mine":"")+'">'+
      '<span class="pvp-tournament-seed">'+String(index+1).padStart(2,"0")+'</span>'+
      '<div><strong>'+hgTournamentEscape(player.nick)+'</strong><small>'+(player.teamLocked?hgTournamentEscape(hgTournamentT("tournament.teamConfirmed","TIME CONFIRMADO")):hgTournamentEscape(hgTournamentT("tournament.teamPending","TIME PENDENTE")))+'</small></div>'+
...
function hgTournamentRenderBracket(tournament,session){
  const box=document.getElementById("hgTournamentBracket");
  if(!box)return;
  const bracket=tournament.bracket;
  if(!bracket||!Array.isArray(bracket.rounds)||!bracket.rounds.length){
    box.innerHTML='<div class="pvp-tournament-empty">'+hgTournamentEscape(hgTournamentT("tournament.bracketWaiting","A chave ainda não foi gerada."))+'</div>';
    return;
  }
  const myNick=String(session.nick||"").toLowerCase();
  box.innerHTML='<div class="pvp-tournament-bracket-scroll">'+bracket.rounds.map(function(round){
    const isFinal=Number(round.round)===Number(bracket.roundCount);
    return '<section class="pvp-tournament-round">'+
      '<div class="pvp-tournament-round-title"><small>ROUND '+Number(round.round)+'</small><strong>'+(isFinal?hgTournamentEscape(hgTournamentT("tournament.final","FINAL")):hgTournamentEscape(hgTournamentT("tournament.round","RODADA {n}",{n:round.round})))+'</strong></div>'+
      '<div class="pvp-tournament-round-matches">'+
      (round.matches||[]).map(function(match){
        const p1=match.player1, p2=match.player2, winner=match.winner;
        const mine=(p1&&String(p1.nick).toLowerCase()===myNick)||(p2&&String(p2.nick).toLowerCase()===myNick);
...
function hgTournamentRenderMatchPanel(tournament,session){
  const box=document.getElementById("hgTournamentMatchPanel");
  if(!box)return;
  const player=hgTournamentFindPlayer(tournament,session);
  if(!player){
    box.innerHTML=
      '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.matchKicker","MATCH // NEXT"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.yourMatch","SUA PRÓXIMA PARTIDA"))+'</strong></div></div>'+
      '<div class="pvp-tournament-empty">'+hgTournamentEscape(hgTournamentT("tournament.joinToPlay","Entre no torneio para receber suas partidas."))+'</div>';
    return;
  }

  if(player.status==="champion"){
    box.innerHTML=
      '<div class="pvp-tournament-champion"><span>🏆</span><div><small>'+hgTournamentEscape(hgTournamentT("tournament.championKicker","TOURNAMENT COMPLETE"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.youChampion","VOCÊ É O CAMPEÃO!"))+'</strong></div></div>';
    return;
  }
  if(player.status==="eliminated"){
    box.innerHTML=
      '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.matchKicker","MATCH // STATUS"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.eliminatedTitle","TORNEIO ENCERRADO PARA VOCÊ"))+'</strong></div></div>'+
      '<div class="pvp-tournament-alert bad">'+hgTournamentEscape(hgTournamentT("tournament.eliminatedHint","Você foi eliminado. A chave continua disponível para acompanhar o restante do torneio."))+'</div>';
    return;
  }
  ...
  if(!tournament.bracket){
    box.innerHTML=
      '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.matchKicker","MATCH // NEXT"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.yourMatch","SUA PRÓXIMA PARTIDA"))+'</strong></div></div>'+
      '<div class="pvp-tournament-empty">'+hgTournamentEscape(player.teamLocked?hgTournamentT("tournament.waitBracket","Time confirmado. Aguarde o organizador gerar a chave."):hgTournamentT("tournament.lockTeamFirst","Confirme seu time antes do fechamento das inscrições."))+'</div>';
    return;
  }

  const match=hgTournamentCurrentMatch(tournament,player);
  if(!match){
    box.innerHTML=
      '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.matchKicker","MATCH // NEXT"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.yourMatch","SUA PRÓXIMA PARTIDA"))+'</strong></div></div>'+
      '<div class="pvp-tournament-empty">'+hgTournamentEscape(hgTournamentT("tournament.waitNextMatch","Aguardando atualização da chave."))+'</div>';
    return;
  }

  const meIsP1=Number(match.player1&&match.player1.id)===Number(player.id);
  const opponent=meIsP1?match.player2:match.player1;
  const ready=match.status==="ready"&&match.player1&&match.player2&&!match.winner;
  let stateText="";
  if(ready)stateText=hgTournamentT("tournament.matchReadyHint","Seu confronto está pronto. A Challenge Room será aberta automaticamente.");
  else if(!opponent)stateText=hgTournamentT("tournament.waitOpponent","Você avançou e está aguardando o adversário da próxima rodada.");
  else stateText=hgTournamentT("tournament.waitMatch","Aguardando esta partida ficar disponível.");

  box.innerHTML=
    '<div class="pvp-tournament-panel-title"><div><small>'+hgTournamentEscape(hgTournamentT("tournament.matchKicker","MATCH // NEXT"))+'</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.yourMatch","SUA PRÓXIMA PARTIDA"))+'</strong></div><b class="pvp-tournament-match-number">#'+Number(match.id)+'</b></div>'+
    '<div class="pvp-tournament-next-versus">'+
      '<div class="mine"><small>'+hgTournamentEscape(hgTournamentT("tournament.you","VOCÊ"))+'</small><strong>'+hgTournamentEscape(player.nick)+'</strong></div>'+
      '<span>VS</span>'+
      '<div><small>'+hgTournamentEscape(hgTournamentT("tournament.opponent","ADVERSÁRIO"))+'</small><strong>'+hgTournamentEscape(opponent?opponent.nick:hgTournamentT("tournament.waitingPlayer","AGUARDANDO..."))+'</strong></div>'+
    '</div>'+
    '<p class="pvp-tournament-muted">'+hgTournamentEscape(stateText)+'</p>'+
    (ready?'<button type="button" class="pvp-action-btn pvp-action-success pvp-tournament-enter-match" onclick="hgTournamentEnterMatch('+Number(match.id)+')">'+hgTournamentEscape(hgTournamentT("tournament.enterFight","ENTRAR NA LUTA"))+'</button>':'');
}

function hgTournamentRenderDashboard(tournament){
  hgTournamentData=tournament;
  const landing=document.getElementById("hgTournamentLanding");
  const dashboard=document.getElementById("hgTournamentDashboard");
  if(landing)landing.hidden=true;
  if(dashboard)dashboard.hidden=false;
  const session=hgTournamentGetSession(tournament.id);
  hgTournamentRenderTop(tournament,session);
  hgTournamentRenderPlayerPanel(tournament,session);
  hgTournamentRenderMatchPanel(tournament,session);
  hgTournamentRenderOrganizerPanel(tournament,session);
  hgTournamentRenderParticipants(tournament,session);
...
async function hgTournamentOpen(id,silent){
  ...
  if(!/^HG[A-Z2-9]{6}$/.test(key)){
    if(!silent)hgTournamentNotice(hgTournamentT("tournament.error.invalidCode","Tournament ID inválido."),"bad");
    return false;
  }
  if(hgTournamentLoading)return false;
  hgTournamentSetBusy(true);
  if(!silent)hgTournamentNotice(hgTournamentT("tournament.loading","Carregando torneio..."),"info");
  try{
    ...
    hgTournamentSetBusy(false);
  }
}

async function abrirPvpTournament(id){
  fecharPvpNavMenu();
  mostrarPagina("pvpPagina",document.getElementById("btnPvp"));
  pvpMostrarView("tournament");
  pvpCriarSlots();
  pvpCarregarDatabase().catch(function(){});
  const key=hgTournamentNormalizeId(id||hgTournamentQueryId()||hgTournamentGetActive());
  if(key)await hgTournamentOpen(key,true);
  if(!hgTournamentData)hgTournamentRenderLanding();
  hgTournamentStartPolling();
}
...
async function hgTournamentCreate(){
  if(hgTournamentLoading)return;
  const name=String(document.getElementById("hgTournamentCreateName")?.value||"").trim().slice(0,60);
  const organizer=String(document.getElementById("hgTournamentCreateOrganizer")?.value||"").replace(/[^A-Za-z0-9]/g,"").slice(0,16);
  const maxPlayers=Number(document.getElementById("hgTournamentCreatePlayers")?.value||0);
  const stage=String(document.getElementById("hgTournamentCreateStage")?.value||"");
  if(name.length<3){hgTournamentNotice(hgTournamentT("tournament.error.name","Digite um nome de torneio com pelo menos 3 caracteres."),"bad");return}
  if(!/^[A-Za-z0-9]{3,16}$/.test(organizer)){hgTournamentNotice(hgTournamentT("tournament.error.nick","Use um nick com 3 a 16 caracteres, somente letras e números."),"bad");return}
  if(!Number.isInteger(maxPlayers)||maxPlayers<2||maxPlayers>256){hgTournamentNotice(hgTournamentT("tournament.error.players","Use uma quantidade de participantes entre 2 e 256."),"bad");return}
  hgTournamentSetBusy(true);
  hgTournamentNotice(hgTournamentT("tournament.creating","Criando torneio..."),"info");
  try{
    const data=await hgTournamentRequest("/api/tournaments",{
      method:"POST",
      body:JSON.stringify({name:name,organizer:organizer,maxPlayers:maxPlayers,rules:{stage:stage}})
    });
    const id=data.tournament.id;
    const organizerPlays=document.getElementById("hgTournamentCreateOrganizerPlays")?.checked!==false;
    let tournament=data.tournament;
    const sessionPatch={organizerToken:data.organizerToken,organizerNick:organizer};

    if(organizerPlays){
      const joined=await hgTournamentRequest("/api/tournaments/"+encodeURIComponent(id)+"/join",{
        method:"POST",
        body:JSON.stringify({nick:organizer})
      });
      sessionPatch.playerToken=joined.playerToken;
      sessionPatch.nick=organizer;
      tournament=joined.tournament||tournament;
      try{localStorage.setItem(PVP_MATCH_NICK_KEY,organizer)}catch(erro){}
    }

    hgTournamentSaveSession(id,sessionPatch);
...
async function hgTournamentJoin(id,nick){
  const key=hgTournamentNormalizeId(id);
  const cleanNick=String(nick||"").replace(/[^A-Za-z0-9]/g,"").slice(0,16);
  if(!/^HG[A-Z2-9]{6}$/.test(key)){hgTournamentNotice(hgTournamentT("tournament.error.invalidCode","Tournament ID inválido."),"bad");return false}
  if(!/^[A-Za-z0-9]{3,16}$/.test(cleanNick)){hgTournamentNotice(hgTournamentT("tournament.error.nick","Use um nick com 3 a 16 caracteres, somente letras e números."),"bad");return false}
  hgTournamentSetBusy(true);
  hgTournamentNotice(hgTournamentT("tournament.joining","Entrando no torneio..."),"info");
  try{
    const data=await hgTournamentRequest("/api/tournaments/"+encodeURIComponent(key)+"/join",{
      method:"POST",
      body:JSON.stringify({nick:cleanNick})
    });
    hgTournamentSaveSession(key,{playerToken:data.playerToken,nick:cleanNick});
    try{localStorage.setItem(PVP_MATCH_NICK_KEY,cleanNick)}catch(erro){}
    hgTournamentSetActive(key);
    hgTournamentSetQuery(key);
    hgTournamentRenderDashboard(data.tournament);
    hgTournamentNotice(hgTournamentT("tournament.joined","Inscrição realizada. Agora confirme seu time antes do fechamento das inscrições."),"ok");
    return true;
  }catch(erro){
    hgTournamentNotice(erro.message,"bad");
    return false;
...
function hgTournamentJoinFromLanding(){
  const id=document.getElementById("hgTournamentJoinId")?.value||"";
  const nick=document.getElementById("hgTournamentJoinNick")?.value||"";
  return hgTournamentJoin(id,nick);
}

function hgTournamentJoinActive(){
  if(!hgTournamentData)return;
  const nick=document.getElementById("hgTournamentInlineNick")?.value||"";
  return hgTournamentJoin(hgTournamentData.id,nick);
}

function hgTournamentViewFromLanding(){
  const id=document.getElementById("hgTournamentJoinId")?.value||"";
  return hgTournamentOpen(id,false);
}

async function hgTournamentRefresh(){
  const id=hgTournamentData&&hgTournamentData.id?hgTournamentData.id:(hgTournamentQueryId()||hgTournamentGetActive());
  if(!id){hgTournamentRenderLanding();return}
  await hgTournamentOpen(id,false);
}

async function hgTournamentConfirmTeam(){
  if(!hgTournamentData)return;
  const session=hgTournamentGetSession(hgTournamentData.id);
  if(!session.playerToken){hgTournamentNotice(hgTournamentT("tournament.error.noPlayerAccess","Este navegador não possui o acesso de jogador deste torneio."),"bad");return}
  hgTournamentNotice(hgTournamentT("tournament.checkingTeam","Validando time PvP..."),"info");
  try{
    await pvpCarregarDatabase();
    pvpCriarSlots();
    const stage=hgTournamentData.rules&&hgTournamentData.rules.stage?hgTournamentData.rules.stage:"";
    const team=pvpMatchTeamAtual();
    if(!pvpMatchTeamValido(team,stage)){
      hgTournamentNotice(hgTournamentT("tournament.error.invalidLocalTeam","O time atual não está pronto para a Stage deste torneio. São necessários 8 builds completos e 3 Battle Cards."),"bad");
      hgTournamentRenderPlayerPanel(hgTournamentData,session);
      return;
    }
    if(!confirm(hgTournamentT("tournament.confirmLockDialog","Confirmar este time? Depois disso ele ficará travado para todo o torneio.")))return;
    const data=await hgTournamentRequest("/api/tournaments/"+encodeURIComponent(hgTournamentData.id)+"/team/lock",{
      method:"POST",
...
async function hgTournamentEditTeam(){
  if(!hgTournamentData)return;
  const session=hgTournamentGetSession(hgTournamentData.id);
  if(!session.playerToken){
    hgTournamentNotice(hgTournamentT("tournament.error.noPlayerAccess","Este navegador não possui o acesso de jogador deste torneio."),"bad");
    return;
  }
  const stage=hgTournamentData.rules&&hgTournamentData.rules.stage?hgTournamentData.rules.stage:"Mega";
  const level=PVP_STAGE_LEVEL[stage]||100;
  try{await pvpCarregarDatabase()}catch(erro){}
  pvpCriarSlots();
  const current=pvpMatchTeamAtual();
  if(current&&current.stage&&current.stage!==stage){
    const ok=confirm(hgTournamentT("tournament.changeStageDialog","Este torneio exige {stage}. Trocar o PvP Builder para essa Stage? Digimons de outra Stage serão removidos do time local.",{stage:hgTournamentStageLabel(stage)}));
    if(!ok)return;
  }
  pvpSelecionarStage(stage,level);
  hgTournamentSetBuilderReturn({
    tournamentId:hgTournamentData.id,
    stage:stage,
    startedAt:Date.now()
  });
  abrirPvpBuild();
  hgTournamentUpdateBuilderSaveButton(true);
  hgTournamentNotice("","");
}

async function hgTournamentOrganizerJoinAsPlayer(){
  if(!hgTournamentData)return false;
  const session=hgTournamentGetSession(hgTournamentData.id);
  if(!session.organizerToken){
    hgTournamentNotice(hgTournamentT("tournament.error.noOrganizerAccess","Este navegador não possui o acesso do organizador."),"bad");
    return false;
  }
  if(session.playerToken){
    hgTournamentNotice(hgTournamentT("tournament.organizerAlreadyPlayer","Você já está inscrito como jogador neste torneio."),"info");
    return true;
  }
  if(Number(hgTournamentData.playerCount||0)>=Number(hgTournamentData.maxPlayers||0)){
    hgTournamentNotice(hgTournamentT("tournament.error.fullOrganizer","O torneio já está lotado. O organizador não pode entrar como jogador sem uma vaga livre."),"bad");
    return false;
  }
...
async function hgTournamentCopyText(text,success){
  ...
  try{
    ...
    hgTournamentNotice(success||hgTournamentT("tournament.copied","Copiado."),"ok");
  }catch(erro){
    hgTournamentNotice(hgTournamentT("tournament.error.copy","Não foi possível copiar automaticamente."),"bad");
  }
}

function hgTournamentCopyInvite(){
  if(hgTournamentData)hgTournamentCopyText(hgTournamentInviteUrl(hgTournamentData.id),hgTournamentT("tournament.inviteCopied","Link do torneio copiado."));
}

function hgTournamentCopyCode(){
  if(hgTournamentData)hgTournamentCopyText(hgTournamentData.id,hgTournamentT("tournament.codeCopied","Tournament ID copiado."));
}

async function hgTournamentEnterMatch(matchId){
  if(!hgTournamentData)return;
  const session=hgTournamentGetSession(hgTournamentData.id);
  if(!session.playerToken){hgTournamentNotice(hgTournamentT("tournament.error.noPlayerAccess","Este navegador não possui o acesso de jogador deste torneio."),"bad");return}
  hgTournamentNotice(hgTournamentT("tournament.openingMatch","Abrindo Challenge Room..."),"info");
  try{
    const data=await hgTournamentRequest("/api/tournaments/"+encodeURIComponent(hgTournamentData.id)+"/matches/"+Number(matchId)+"/access",{
      method:"POST",
      headers:{Authorization:"Bearer "+session.playerToken}
    });
    hgTournamentSaveMatchContext({
      tournamentId:hgTournamentData.id,
    ...
    document.body.classList.add("hg-tournament-match-active");
    fecharPvpNavMenu();
    mostrarPagina("pvpPagina",document.getElementById("btnPvp"));
    pvpMostrarView("match");
    pvpMatchSairSala(true);
    pvpMatchRole=data.role;
...
function hgTournamentMatchContextValid(state){
  const ctx=hgTournamentMatchContext||hgTournamentReadMatchContext();
  if(!ctx||!state)return false;
  if(!ctx.tournamentId||!ctx.matchId||!ctx.playerToken||!ctx.roomId)return false;
  return String(ctx.roomId).toUpperCase()===String(state.roomId||"").toUpperCase();
}

function hgTournamentDecoratePostMatch(state){
  if(!hgTournamentMatchContextValid(state))return;
  if(!state||state.phase!=="finished"||!state.battle||!state.battle.winner)return;
  document.body.classList.add("hg-tournament-match-active");
  ...
  const ctx=hgTournamentMatchContext||{};
  if(status){
    status.className="pvp-postmatch-status tournament";
    if(ctx.reportError)status.textContent=hgTournamentT("tournament.resultError","A partida terminou, mas houve erro ao sincronizar o resultado: {error}",{error:ctx.reportError});
    else if(ctx.reported)status.textContent=ctx.reportFinalized
      ?hgTournamentT("tournament.resultFinalized","Resultado confirmado. A chave foi atualizada.")
      :hgTournamentT("tournament.resultReported","Resultado enviado. Aguardando a confirmação do outro jogador.");
    else status.textContent=hgTournamentT("tournament.resultSending","Sincronizando o resultado com o torneio...");
  }
  if(actions){
    actions.innerHTML='<button type="button" class="pvp-action-btn pvp-action-success" onclick="hgTournamentBackToTournament()">'+hgTournamentEscape(hgTournamentT("tournament.backToTournament","VOLTAR AO TORNEIO"))+'</button>';
  }
}

async function hgTournamentReportResult(state){
  if(hgTournamentResultBusy||!hgTournamentMatchContextValid(state))return;
  if(!state||state.phase!=="finished"||!state.battle||!["host","guest"].includes(state.battle.winner))return;
  const ctx=hgTournamentMatchContext||hgTournamentReadMatchContext();
  if(!ctx||ctx.reported)return;
  hgTournamentResultBusy=true;
  ctx.reportedWinner=state.battle.winner;
  hgTournamentSaveMatchContext(ctx);
  hgTournamentDecoratePostMatch(state);
  try{
    const data=await hgTournamentRequest("/api/tournaments/"+encodeURIComponent(ctx.tournamentId)+"/matches/"+Number(ctx.matchId)+"/result",{
      method:"POST",
      headers:{Authorization:"Bearer "+ctx.playerToken},
      body:JSON.stringify({winnerRole:state.battle.winner})
    });
    ctx.reported=true;
    ctx.reportFinalized=!!data.finalized;
    ctx.reportError="";
    hgTournamentSaveMatchContext(ctx);
  }catch(erro){
    ctx.reported=false;
    ctx.reportError=erro.message||String(erro);
    hgTournamentSaveMatchContext(ctx);
  }finally{
    hgTournamentResultBusy=false;
    hgTournamentDecoratePostMatch(state);
  }
}

async function hgTournamentBackToTournament(){
  const ctx=hgTournamentMatchContext||hgTournamentReadMatchContext();
  const id=ctx&&ctx.tournamentId?ctx.tournamentId:(hgTournamentGetActive()||hgTournamentQueryId());
  pvpMatchSairSala(true);
  document.body.classList.remove("hg-tournament-match-active");
  hgTournamentSaveMatchContext(null);
  await abrirPvpTournament(id);
  await hgTournamentRefresh();
}

function hgTournamentSetBuilderReturn(data){
  try{
    if(data&&data.tournamentId)localStorage.setItem(HG_TOURNAMENT_BUILDER_RETURN_KEY,JSON.stringify(data));
    else localStorage.removeItem(HG_TOURNAMENT_BUILDER_RETURN_KEY);
  }catch(erro){}
}

function hgTournamentReadBuilderReturn(){
  try{
    const data=JSON.parse(localStorage.getItem(HG_TOURNAMENT_BUILDER_RETURN_KEY)||"null");
    if(!data||!data.tournamentId)return null;
    if(data.startedAt&&Date.now()-Number(data.startedAt)>4*60*60*1000){
      localStorage.removeItem(HG_TOURNAMENT_BUILDER_RETURN_KEY);
      return null;
    }
    return data;
  }catch(erro){return null}
...
function hgTournamentClearBuilderReturn(){
  hgTournamentSetBuilderReturn(null);
  hgTournamentUpdateBuilderSaveButton(false);
}

function hgTournamentUpdateBuilderSaveButton(active){
  const btn=document.getElementById("pvpSaveTeamBtn");
  if(!btn)return;
  if(active){
    btn.textContent=hgTournamentT("tournament.saveBack","SALVAR TIME → TORNEIO");
    btn.setAttribute("data-hg-tournament-return","1");
  }else if(btn.getAttribute("data-hg-tournament-return")==="1"){
    btn.textContent="SAVE TEAM → MATCH";
    btn.removeAttribute("data-hg-tournament-return");
  }
}

async function hgTournamentReturnFromBuilder(){
  const ctx=hgTournamentReadBuilderReturn();
  if(!ctx)return false;
  const id=hgTournamentNormalizeId(ctx.tournamentId);
  hgTournamentClearBuilderReturn();
  if(!id)return false;

  await abrirPvpTournament(id);
  if(!hgTournamentData||hgTournamentData.id!==id)return true;

  const session=hgTournamentGetSession(id);
  const player=hgTournamentFindPlayer(hgTournamentData,session);
  if(player&&player.teamLocked){
    hgTournamentNotice(hgTournamentT("tournament.teamLockedSuccess","Time confirmado e travado com sucesso."),"ok");
    return true;
  }

  hgTournamentNotice(hgTournamentT("tournament.returnedFromBuilder","Time salvo. Falta apenas confirmar e travar o time neste torneio."),"ok");
  if(hgTournamentData.status==="registration"&&session.playerToken){
    setTimeout(function(){hgTournamentConfirmTeam()},180);
  }
  return true;
}

const _hgTournamentPvpSalvarTimeOriginal=pvpSalvarTime;
pvpSalvarTime=function(){
  const ctx=hgTournamentReadBuilderReturn();
  if(!ctx)return _hgTournamentPvpSalvarTimeOriginal.apply(this,arguments);
  if(!pvpTodosBuildsConcluidos()){
    alert("Conclua os 8 builds antes de salvar o time para Match.");
    return;
  }
...
const _hgTournamentImportedTeamOriginal=pvpMatchComTimeImportado;
pvpMatchComTimeImportado=function(){
  const ctx=hgTournamentReadBuilderReturn();
  if(!ctx)return _hgTournamentImportedTeamOriginal.apply(this,arguments);
  if(!pvpTodosBuildsConcluidos()){
    alert("Este time possui builds incompletos. Edite o time antes de seguir para Match.");
    return;
  }
...
pvpMatchReceberEstado=function(state){
  const out=_hgTournamentReceiveState.apply(this,arguments);
  if(hgTournamentMatchContextValid(state)){
    document.body.classList.add("hg-tournament-match-active");
    hgTournamentDecoratePostMatch(state);
    hgTournamentReportResult(state);
  }
  return out;
};

const _hgTournamentSairSala=pvpMatchSairSala;
pvpMatchSairSala=function(silencioso){
  if(hgTournamentMatchContext&&!silencioso){
    hgTournamentBackToTournament();
    return;
  }
  const out=_hgTournamentSairSala.apply(this,arguments);
  if(!hgTournamentMatchContext)document.body.classList.remove("hg-tournament-match-active");
  return out;
};

document.addEventListener("hg:languagechange",function(){
  if(hgTournamentData)hgTournamentRenderDashboard(hgTournamentData);
  else hgTournamentRenderSaved();
});

document.addEventListener("DOMContentLoaded",function(){
  hgTournamentRenderSaved();
  hgTournamentUpdateBuilderSaveButton(!!hgTournamentReadBuilderReturn());
  const id=hgTournamentQueryId();
  const route=String(window.location.hash||"").replace(/^#/,"").split("/")[0].toLowerCase();
  if(id&&(!route||route==="pvp")){
    setTimeout(function(){abrirPvpTournament(id)},140);
  }
  const ctx=hgTournamentReadMatchContext();
  if(ctx&&ctx.tournamentId&&ctx.roomId){
    hgTournamentMatchContext=ctx;
  }
});
...
/* ======================================================================
   HG TOURNAMENT V2 VISUAL — COMMAND CENTER + TEAM MODAL + GLOBAL STREAMER
   Frontend-only UX layer. Tournament/Challenge Room mechanics untouched.
   ====================================================================== */

function hgTournamentStreamerActiveV2(){
  try{
    if(typeof pvpMatchStreamerModeAtivoV52==="function")return !!pvpMatchStreamerModeAtivoV52();
    if(typeof pvpMatchStreamerModeAtivo==="function")return !!pvpMatchStreamerModeAtivo();
  }catch(erro){}
  return false;
}

function hgTournamentEnsureStreamerFloatV2(){
  let btn=document.getElementById("hgTournamentStreamerFloatV2");
  if(btn)return btn;
  btn=document.createElement("button");
  btn.id="hgTournamentStreamerFloatV2";
  btn.type="button";
  btn.className="hg-tournament-streamer-float-v2";
  btn.onclick=function(){hgTournamentToggleStreamerModeV2(false)};
  btn.innerHTML='<span>●</span><b>STREAMER ON</b><small>'+hgTournamentEscape(hgTournamentT("tournament.streamerExit","DESATIVAR"))+'</small>';
  document.body.appendChild(btn);
  return btn;
}

function hgTournamentApplyStreamerHubV2(){
  const active=hgTournamentStreamerActiveV2();
  const pvp=document.getElementById("pvpPagina");
  const pvpVisible=!!(pvp&&pvp.classList.contains("ativa"));
  document.body.classList.toggle("hg-pvp-streamer-global-v2",active&&pvpVisible);

  const btn=document.getElementById("hgTournamentStreamerBtn");
  if(btn){
    btn.classList.toggle("ativo",active);
    btn.setAttribute("aria-pressed",active?"true":"false");
    const state=btn.querySelector("b");
  ...
  const floating=hgTournamentEnsureStreamerFloatV2();
  floating.hidden=!(active&&pvpVisible);
}

function hgTournamentToggleStreamerModeV2(force){
  const next=typeof force==="boolean"?force:!hgTournamentStreamerActiveV2();
  if(typeof pvpMatchToggleStreamerMode==="function")pvpMatchToggleStreamerMode(next);
  else{
    try{localStorage.setItem(PVP_MATCH_STREAMER_KEY,next?"1":"0")}catch(erro){}
  }
...
if(typeof pvpMatchAplicarStreamerMode==="function"){
  const _hgTournamentStreamerApplyBaseV2=pvpMatchAplicarStreamerMode;
  pvpMatchAplicarStreamerMode=function(){
    const out=_hgTournamentStreamerApplyBaseV2.apply(this,arguments);
    hgTournamentApplyStreamerHubV2();
    return out;
  };
}

if(typeof pvpMostrarView==="function"){
  const _hgTournamentPvpMostrarViewV2=pvpMostrarView;
  pvpMostrarView=function(){
    const out=_hgTournamentPvpMostrarViewV2.apply(this,arguments);
    requestAnimationFrame(hgTournamentApplyStreamerHubV2);
    return out;
  };
}

if(typeof mostrarPagina==="function"){
  const _hgTournamentMostrarPaginaV2=mostrarPagina;
  mostrarPagina=function(){
    const out=_hgTournamentMostrarPaginaV2.apply(this,arguments);
    requestAnimationFrame(hgTournamentApplyStreamerHubV2);
    return out;
  };
}

function hgTournamentOrganizerViewPlayerV2(playerId){
  const view=hgTournamentOrganizerView;
  if(!view||!Array.isArray(view.players))return null;
  return view.players.find(function(p){return String(p.id)===String(playerId)})||null;
}

function hgTournamentHasMapValuesV2(map){
  return !!(map&&typeof map==="object"&&Object.keys(map).some(function(k){return Number(map[k])!==0&&map[k]!=null&&map[k]!==""}));
}

function hgTournamentTeamBadgeV2(label,ok,text){
  return '<span class="hg-tour-v2-team-badge '+(ok?'ok':'muted')+'"><b>'+hgTournamentEscape(label)+'</b><small>'+hgTournamentEscape(text|| (ok?'✓':'—'))+'</small></span>';
}

function hgTournamentEnsureTeamModalV2(){
  let modal=document.getElementById("hgTournamentTeamModalV2");
  if(modal)return modal;
  modal=document.createElement("div");
  modal.id="hgTournamentTeamModalV2";
  modal.className="hg-tour-v2-modal";
  modal.hidden=true;
  modal.innerHTML=
    '<div class="hg-tour-v2-modal-backdrop" onclick="hgTournamentCloseTeamModalV2()"></div>'+ 
    '<section class="hg-tour-v2-modal-card tech-corners" role="dialog" aria-modal="true">'+
      '<header class="hg-tour-v2-modal-head">'+
        '<div><small>TEAM AUDIT // ORGANIZER</small><h3 id="hgTournamentTeamModalTitleV2">TIME</h3><p id="hgTournamentTeamModalMetaV2"></p></div>'+ 
        '<button type="button" class="pvp-action-btn pvp-action-secondary" onclick="hgTournamentCloseTeamModalV2()">'+hgTournamentEscape(hgTournamentT("tournament.close","FECHAR"))+' ✕</button>'+ 
      '</header>'+ 
      '<div id="hgTournamentTeamModalSummaryV2" class="hg-tour-v2-modal-summary"></div>'+ 
      '<div class="hg-tour-v2-modal-workspace">'+
        '<div id="hgTournamentTeamGridV2" class="hg-tour-v2-team-grid"></div>'+ 
        '<aside id="hgTournamentTeamDetailV2" class="hg-tour-v2-team-detail" hidden></aside>'+ 
      '</div>'+ 
    '</section>';
...
function hgTournamentCloseTeamModalV2(){
  const modal=document.getElementById("hgTournamentTeamModalV2");
  if(modal)modal.hidden=true;
  document.body.classList.remove("hg-tour-v2-modal-open");
}

function hgTournamentOpenDeckDetailV2(playerId){
  const player=hgTournamentOrganizerViewPlayerV2(playerId);
  const team=player&&player.team;
  const detail=document.getElementById("hgTournamentTeamDetailV2");
  const modal=document.getElementById("hgTournamentTeamModalV2");
  if(!detail||!modal||!team)return;
  const deck=team.teamDeck&&typeof team.teamDeck==="object"?team.teamDeck:{};
  const cards=Array.isArray(team.battleCards)?team.battleCards.filter(Boolean):[];
  detail.hidden=false;
  modal.classList.add("detail-open");
  detail.innerHTML=
    '<div class="hg-tour-v2-detail-head"><div><small>GLOBAL LOADOUT</small><strong>'+hgTournamentEscape(hgTournamentT("tournament.deckDetails","DECK / ATRIBUTOS"))+'</strong></div><button type="button" onclick="hgTournamentCloseTeamDetailV2()">✕</button></div>'+ 
    '<div class="hg-tour-v2-detail-list">'+
      '<div><b>BATTLE CARDS</b><span>'+hgTournamentEscape(cards.length?cards.map(function(x){return String(x).toUpperCase()}).join(" · "):"—")+'</span></div>'+ 
      '<div><b>BUFF DECK</b><span>'+hgTournamentEscape(hgTournamentMapSummary(deck.buff))+'</span></div>'+ 
      '<div><b>ATTR BOOST</b><span>'+hgTournamentEscape(hgTournamentMapSummary(deck.attrBoost))+'</span></div>'+ 
      '<div><b>ATTR REDUCE</b><span>'+hgTournamentEscape(hgTournamentMapSummary(deck.attrReduce))+'</span></div>'+ 
    '</div>';
...
function hgTournamentOpenSlotDetailV2(playerId,index){
  const player=hgTournamentOrganizerViewPlayerV2(playerId);
  const team=player&&player.team;
  const slots=team&&Array.isArray(team.slots)?team.slots:[];
  const slot=slots[Number(index)]||null;
  const detail=document.getElementById("hgTournamentTeamDetailV2");
  const modal=document.getElementById("hgTournamentTeamModalV2");
  if(!detail||!modal||!slot)return;
  const build=slot.build&&typeof slot.build==="object"?slot.build:{};
  detail.hidden=false;
  modal.classList.add("detail-open");
  detail.innerHTML=
    '<div class="hg-tour-v2-detail-head"><div><small>#'+String(Number(index)+1).padStart(2,"0")+' · '+hgTournamentEscape(slot.hgid||"—")+'</small><strong>'+hgTournamentEscape(slot.digimon||slot.hgid||"DIGIMON")+'</strong></div><button type="button" onclick="hgTournamentCloseTeamDetailV2()">✕</button></div>'+ 
    '<div class="hg-tour-v2-detail-list">'+
      '<div><b>BABY CORRECTION</b><span>'+hgTournamentEscape(hgTournamentMapSummary(build.baby))+'</span></div>'+ 
      '<div><b>TETRIS</b><span>'+hgTournamentEscape(hgTournamentMapSummary(build.tetris))+'</span></div>'+ 
      '<div><b>SKILL ELEMENTS</b><span>'+hgTournamentEscape(hgTournamentSkillElementsSummary(build.skillElements))+'</span></div>'+ 
      '<div><b>BURST</b><span>S'+Number(build.burstSkill||1)+'</span></div>'+ 
    '</div>';
}

function hgTournamentCloseTeamDetailV2(){
  const detail=document.getElementById("hgTournamentTeamDetailV2");
  const modal=document.getElementById("hgTournamentTeamModalV2");
  if(detail){detail.hidden=true;detail.innerHTML=""}
  if(modal)modal.classList.remove("detail-open");
}

function hgTournamentOpenTeamModalV2(playerId){
  const player=hgTournamentOrganizerViewPlayerV2(playerId);
  if(!player||!player.team)return;
  const modal=hgTournamentEnsureTeamModalV2();
  const team=player.team;
  const slots=Array.isArray(team.slots)?team.slots:[];
  const cards=Array.isArray(team.battleCards)?team.battleCards.filter(Boolean):[];
  const title=document.getElementById("hgTournamentTeamModalTitleV2");
  const meta=document.getElementById("hgTournamentTeamModalMetaV2");
  const summary=document.getElementById("hgTournamentTeamModalSummaryV2");
  const grid=document.getElementById("hgTournamentTeamGridV2");
  if(title)title.textContent=hgTournamentT("tournament.teamOf","TIME DE {name}",{name:player.nick});
  if(meta)meta.textContent=hgTournamentStageLabel(team.stage||"-")+" · "+slots.length+"/8 · "+hgTournamentT("tournament.teamLocked","TIME TRAVADO");
  if(summary)summary.innerHTML=
    '<span><small>STAGE</small><strong>'+hgTournamentEscape(hgTournamentStageLabel(team.stage||"-"))+'</strong></span>'+ 
    '<span><small>BATTLE CARDS</small><strong>'+hgTournamentEscape(cards.length?cards.map(function(x){return String(x).toUpperCase()}).join(" · "):"—")+'</strong></span>'+ 
    '<span><small>DIGIMONS</small><strong>'+slots.length+' / 8</strong></span>'+ 
    '<button type="button" onclick="hgTournamentOpenDeckDetailV2('+Number(player.id)+')">'+hgTournamentEscape(hgTournamentT("tournament.deckDetails","DECK / ATRIBUTOS"))+'</button>';

  if(grid)grid.innerHTML=slots.map(function(slot,index){
    const build=slot&&slot.build&&typeof slot.build==="object"?slot.build:{};
    const baby=hgTournamentHasMapValuesV2(build.baby);
    const tetris=hgTournamentHasMapValuesV2(build.tetris);
    const skillText=hgTournamentSkillElementsSummary(build.skillElements);
    return '<article class="hg-tour-v2-team-card">'+
      '<div class="hg-tour-v2-team-card-head"><span>#'+String(index+1).padStart(2,"0")+'</span><div><strong>'+hgTournamentEscape(slot&&slot.digimon?slot.digimon:(slot&&slot.hgid?slot.hgid:"—"))+'</strong><small>'+hgTournamentEscape(slot&&slot.hgid?slot.hgid:"—")+'</small></div></div>'+ 
      '<div class="hg-tour-v2-team-badges">'+
        hgTournamentTeamBadgeV2("BABY",baby,baby?"✓":"—")+
        hgTournamentTeamBadgeV2("TETRIS",tetris,tetris?"✓":"—")+
        hgTournamentTeamBadgeV2("BURST",true,"S"+Number(build.burstSkill||1))+
      '</div>'+ 
      '<p title="'+hgTournamentEscape(skillText)+'">'+hgTournamentEscape(skillText)+'</p>'+ 
      '<button type="button" onclick="hgTournamentOpenSlotDetailV2('+Number(player.id)+','+Number(index)+')">'+hgTournamentEscape(hgTournamentT("tournament.details","DETALHES"))+'</button>'+ 
    '</article>';
  }).join("");

  hgTournamentCloseTeamDetailV2();
  modal.hidden=false;
  document.body.classList.add("hg-tour-v2-modal-open");
}

/* Organizer side is now command-only. Team inspection lives in the full-width participant list. */
hgTournamentRenderOrganizerTeams=function(view){
  if(!view||!Array.isArray(view.players))return "";
  const ready=view.players.filter(function(p){return !!p.teamLocked}).length;
  return '<div class="hg-tour-v2-audit-hint"><span>✓</span><div><strong>'+ready+' / '+view.players.length+' '+hgTournamentEscape(hgTournamentT("tournament.teamsLockedShort","TIMES TRAVADOS"))+'</strong><small>'+hgTournamentEscape(hgTournamentT("tournament.auditHintV2","Abra VER TIME na lista de participantes para auditar sem sair desta tela."))+'</small></div></div>';
};
...
hgTournamentRenderParticipants=function(tournament,session){
  ...
  box.innerHTML=players.map(function(player,index){
    ...
    const canView=!!(organizer&&audit&&audit.teamLocked&&audit.team);
    return '<article class="pvp-tournament-player-row '+(mine?'mine':'')+'">'+
      '<span class="pvp-tournament-seed">'+String(index+1).padStart(2,"0")+'</span>'+ 
      ...
      '<b class="pvp-tournament-mini-status '+hgTournamentStatusClass(player.status)+'">'+hgTournamentEscape(hgTournamentPlayerStatusLabel(player.status))+'</b>'+ 
      (canView?'<button type="button" class="hg-tour-v2-view-team-btn" onclick="hgTournamentOpenTeamModalV2('+Number(player.id)+')">'+hgTournamentEscape(hgTournamentT("tournament.viewTeam","VER TIME"))+'</button>':'')+
    '</article>';
  }).join("");
};

function hgTournamentApplyRoleLayoutV2(tournament){
  const dash=document.getElementById("hgTournamentDashboard");
  if(!dash||!tournament)return;
  const session=hgTournamentGetSession(tournament.id);
  const player=hgTournamentFindPlayer(tournament,session);
  const organizer=!!session.organizerToken;
  ["registration","bracket_ready","in_progress","finished"].forEach(function(s){dash.classList.remove("hg-status-"+s)});
  dash.classList.toggle("hg-organizer-mode",organizer);
  dash.classList.toggle("hg-player-mode",!!player);
  dash.classList.toggle("hg-viewer-mode",!player&&!organizer);
  dash.classList.toggle("hg-organizer-not-player",organizer&&!player);
  dash.classList.add("hg-status-"+String(tournament.status||"registration"));

  const idLine=dash.querySelector(".pvp-tournament-id-line");
  if(idLine){
    let badge=idLine.querySelector(".hg-tour-v2-role-badge");
    if(!badge){badge=document.createElement("b");badge.className="hg-tour-v2-role-badge";idLine.appendChild(badge)}
    if(organizer)badge.textContent=hgTournamentT("tournament.roleOrganizer","ORGANIZADOR");
    else if(player)badge.textContent=hgTournamentT("tournament.rolePlayer","JOGADOR");
    else badge.textContent=hgTournamentT("tournament.roleViewer","ESPECTADOR");
  }
}

const _hgTournamentRenderDashboardV2=hgTournamentRenderDashboard;
hgTournamentRenderDashboard=function(tournament){
  const out=_hgTournamentRenderDashboardV2.apply(this,arguments);
  hgTournamentApplyRoleLayoutV2(tournament);
  hgTournamentApplyStreamerHubV2();
  return out;
};

const _hgTournamentRenderLandingV2=hgTournamentRenderLanding;
hgTournamentRenderLanding=function(){
  const out=_hgTournamentRenderLandingV2.apply(this,arguments);
  requestAnimationFrame(hgTournamentApplyStreamerHubV2);
  return out;
};
...
if(document.readyState==="loading"){
  document.addEventListener("DOMContentLoaded",function(){
    ...
    hgTournamentApplyStreamerHubV2();
    document.addEventListener("keydown",function(e){if(e.key==="Escape")hgTournamentCloseTeamModalV2()});
  });
}else{
  hgTournamentEnsureTeamModalV2();
  hgTournamentApplyStreamerHubV2();
...
/* =========================================================
   ICON LAB V8 — IA LOCAL / MODO DIGI GUIADO / HD RESIZE / EXPORT
...
(function(){
  ...
  function hasBgMark(){return !!(S.bgMask&&S.bgMask.some(value=>value===1))}

  function updateBgButtons(){
    const apply=$("hgIconLabApplyMarkedBg"),clear=$("hgIconLabClearMarkedBg"),auto=$("hgIconLabAutoMarkBg"),marked=hasBgMark();
    if(auto)auto.disabled=S.busy;if(apply)apply.disabled=S.busy||!marked;if(clear)clear.disabled=S.busy||!marked;
  }

  function refreshBgMask(){
    const canvas=$("hgIconLabCanvas");if(!canvas||!canvas.width||!S.bgSeeds.length){S.bgMask=null;updateBgButtons();drawOverlay();return 0}
    if(canvas.width*canvas.height>4000000){message("Recorte o Digi antes de usar a marcação automática.",true);return 0}
    const image=ctx(canvas).getImageData(0,0,canvas.width,canvas.height),data=image.data,w=canvas.width,h=canvas.height;
    const seeds=S.bgSeeds.map(seed=>({
      x:Math.max(0,Math.min(w-1,Math.round(seed.x))),y:Math.max(0,Math.min(h-1,Math.round(seed.y)))
    }));
    const palette=seeds.map(seed=>{const i=(seed.y*w+seed.x)*4;return[data[i],data[i+1],data[i+2]]});
    const mask=new Uint8Array(w*h),queue=new Int32Array(w*h);let head=0,tail=0,count=0;
    const distanceLimit=Math.pow(S.bgTolerance*2.05,2);
    const matches=p=>{
      const i=p*4;if(data[i+3]<=10)return true;
      for(let n=0;n<palette.length;n++){
        const color=palette[n],dr=data[i]-color[0],dg=data[i+1]-color[1],db=data[i+2]-color[2];
        if(dr*dr+dg*dg+db*db<=distanceLimit)return true;
      }
      return false;
    };
    seeds.forEach(seed=>{const p=seed.y*w+seed.x;if(!mask[p]){mask[p]=1;queue[tail++]=p;count++}});
    while(head<tail){
      const p=queue[head++],x=p%w;
      const visit=n=>{if(n>=0&&n<mask.length&&!mask[n]&&matches(n)){mask[n]=1;queue[tail++]=n;count++}};
      if(x>0)visit(p-1);if(x<w-1)visit(p+1);if(p>=w)visit(p-w);if(p<w*(h-1))visit(p+w);
    }
    S.bgMask=mask;updateBgButtons();drawOverlay();
    $("hgIconLabSelection").textContent="Fundo marcado: "+count.toLocaleString("pt-BR")+" px";return count;
  }

  function autoMarkBackground(){
    if(S.busy)return;const canvas=$("hgIconLabCanvas");if(!canvas||!canvas.width)return;
    const inset=Math.min(2,Math.floor((Math.min(canvas.width,canvas.height)-1)/2));
    S.bgSeeds=[{x:inset,y:inset},{x:canvas.width-1-inset,y:inset},{x:inset,y:canvas.height-1-inset},{x:canvas.width-1-inset,y:canvas.height-1-inset}];
    selectTool("wand");const count=refreshBgMask();if(count)message("Fundo marcado em vermelho. Confira e clique em APAGAR MARCADO.");
  }

  function addBgSeed(point){
    if(S.busy)return;S.bgSeeds.push({x:point.x,y:point.y});selectTool("wand");refreshBgMask();
  }

  function clearBgMark(showMessage=true){
    S.bgSeeds=[];S.bgMask=null;updateBgButtons();drawOverlay();
    $("hgIconLabSelection").textContent="Nenhum fundo marcado";if(showMessage)message("Marcação de fundo limpa.");
  }

  function applyMarkedBackground(){
    const canvas=$("hgIconLabCanvas");if(S.busy||!canvas||!hasBgMark())return;
    S.beforeUrl=canvas.toDataURL("image/png");pushHistory();S.restore=cloneCanvas(canvas);
    const image=ctx(canvas).getImageData(0,0,canvas.width,canvas.height);
    for(let p=0;p<S.bgMask.length;p++)if(S.bgMask[p])image.data[p*4+3]=0;
    ctx(canvas).putImageData(image,0,0);S.bgSeeds=[];S.bgMask=null;updateBgButtons();updateCompare();selectTool("erase");
    $("hgIconLabSelection").textContent="Fundo guiado removido";message("Fundo removido sem a IA decidir quais partes pertencem ao Digi.");
  }
  ...
  function drawOverlay(cursor){
    ...
    if(S.bgMask&&S.bgMask.length===overlay.width*overlay.height){
      const preview=o.createImageData(overlay.width,overlay.height);
      for(let p=0;p<S.bgMask.length;p++)if(S.bgMask[p]){const i=p*4;preview.data[i]=255;preview.data[i+1]=38;preview.data[i+2]=91;preview.data[i+3]=112}
      o.putImageData(preview,0,0);
    }
  ...
  function undo(){
    ...
    S.restore=entry.restore||cloneCanvas(canvas);S.crop=null;S.bgSeeds=[];S.bgMask=null;
    $("hgIconLabApplyCrop").disabled=true;$("hgIconLabSelection").textContent="Nenhum recorte selecionado";
    $("hgIconLabUndo").disabled=!S.history.length;updateBgButtons();requestAnimationFrame(syncOverlay);updateDimensions();updateCompare();
  }
  ...
  function bind(){
    ...
    overlay.addEventListener("pointerdown",ev=>{
      ...
      else if(S.tool==="wand"){addBgSeed(p)}
    ...
    $("hgIconLabAutoMarkBg").addEventListener("click",autoMarkBackground);$("hgIconLabApplyMarkedBg").addEventListener("click",applyMarkedBackground);$("hgIconLabClearMarkedBg").addEventListener("click",()=>clearBgMark(true));
